// Package querylanguage provides an expression-based query language for building
// type-safe predicates dynamically at runtime. It defines an AST (abstract syntax
// tree) of expressions that can be evaluated against different backends (e.g., SQL).
//
// The package provides two levels of API:
//
//   - Untyped predicates: [FieldEQ], [FieldGT], [FieldContains], etc. operate on
//     field names and arbitrary values.
//   - Typed predicates: [StringEQ], [IntGT], [BoolEQ], etc. provide compile-time
//     type safety via the [Fielder] interface. These are generated from a template
//     (see internal/gen.go).
//
// Predicates can be composed with [And], [Or], and [Not]:
//
//	p := querylanguage.And(
//	    querylanguage.FieldEQ("status", "active"),
//	    querylanguage.FieldGT("age", 18),
//	)
...
// Valid reports whether the operator is a known, valid operator.
...
type (
	// Expr represents a query language expression node in the AST. All concrete
	// expression types ([BinaryExpr], [UnaryExpr], [NaryExpr], [CallExpr],
	// [Field], [Edge], [Value]) implement this interface.
	Expr interface {
		expr()
		fmt.Stringer
	}

	// P represents a predicate — an expression that evaluates to a boolean.
	// It can be negated with [Not] or its Negate method, and composed with
	// [And] and [Or].
	P interface {
		Expr
		Negate() P
...
// In returns a predicate to check if expression x is in the list of expressions ys.
func In(x Expr, ys ...Expr) P {
	return &BinaryExpr{
		Op: OpIn,
		X:  x,
		Y:  &Value{V: exprsToAny(ys)},
	}
}

// NotIn returns a predicate to check if expression x is not in the list of expressions ys.
func NotIn(x Expr, ys ...Expr) P {
	return &BinaryExpr{
		Op: OpNotIn,
		X:  x,
		Y:  &Value{V: exprsToAny(ys)},
...
func exprsToAny(exprs []Expr) []any {
	vs := make([]any, len(exprs))
	for i, e := range exprs {
		if v, ok := e.(*Value); ok {
			vs[i] = v.V
		} else {
