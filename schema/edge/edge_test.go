...
// TestTypFunction tests the typ function via edge.To/From with various invalid inputs.
func TestTypFunction(t *testing.T) {
	t.Parallel()

	t.Run("nil_type_returns_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("bad", nil).Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "nil")
	})

	t.Run("nil_type_from_returns_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.From("bad", nil).Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "nil")
	})

	t.Run("non_func_type_returns_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("bad", "not_a_func").Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "expected func")
	})

	t.Run("non_func_type_int_returns_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("bad", 42).Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "expected func")
	})

	t.Run("func_with_no_params_returns_error", func(t *testing.T) {
		t.Parallel()
		noParam := func() {}
		desc := edge.To("bad", noParam).Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "no input parameters")
	})

	t.Run("valid_func_type_no_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("posts", Post.Type).Descriptor()
		assert.NoError(t, desc.Err)
		assert.Equal(t, "Post", desc.Type)
	})
}

// TestThroughWithInvalidType tests the Through method with invalid type arguments.
func TestThroughWithInvalidType(t *testing.T) {
	t.Parallel()

	t.Run("assoc_through_nil_type", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("friends", User.Type).Through("friendships", nil).Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "nil")
		// Through struct is still set (with empty type name)
		require.NotNil(t, desc.Through)
		assert.Equal(t, "friendships", desc.Through.N)
	})

	t.Run("assoc_through_non_func_type", func(t *testing.T) {
		t.Parallel()
		desc := edge.To("friends", User.Type).Through("friendships", "bad").Descriptor()
		require.Error(t, desc.Err)
		assert.Contains(t, desc.Err.Error(), "expected func")
	})

	t.Run("assoc_through_preserves_first_error", func(t *testing.T) {
		t.Parallel()
		// If the main edge type already has an error, Through error doesn't overwrite it
		desc := edge.To("friends", nil).Through("friendships", nil).Descriptor()
		require.Error(t, desc.Err)
		// The first error (from To's nil type) should be preserved
		assert.Contains(t, desc.Err.Error(), `edge "friends"`)
	})

	t.Run("inverse_through_nil_type", func(t *testing.T) {
		t.Parallel()
		desc := edge.From("liked_users", User.Type).
			Ref("liked_posts").
			Through("likes", nil).
			Descriptor()
		require.Error(t, desc.Err)
	...
	t.Run("inverse_through_non_func_type", func(t *testing.T) {
		t.Parallel()
		desc := edge.From("liked_users", User.Type).
			Ref("liked_posts").
			Through("likes", 123).
	...
	t.Run("inverse_through_preserves_first_error", func(t *testing.T) {
		t.Parallel()
		desc := edge.From("liked_users", nil).
			Ref("liked_posts").
			Through("likes", nil).
			Descriptor()
		require.Error(t, desc.Err)
		// The first error (from From's nil type) should be preserved
		assert.Contains(t, desc.Err.Error(), `edge "liked_users"`)
	})
}

// TestAnnotationName tests the edge.Annotation Name method.
func TestAnnotationName(t *testing.T) {
	t.Parallel()
	ann := edge.Annotation{}
	assert.Equal(t, "Edges", ann.Name())
}

// TestAnnotationMerge tests the edge.Annotation Merge method.
func TestAnnotationMerge(t *testing.T) {
	t.Parallel()

	t.Run("merge_value_with_struct_tag", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{StructTag: `json:"original"`}
		b := edge.Annotation{StructTag: `json:"override"`}
		result := a.Merge(b)
		ann, ok := result.(edge.Annotation)
		require.True(t, ok)
		assert.Equal(t, `json:"override"`, ann.StructTag)
	})

	t.Run("merge_pointer_with_struct_tag", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{StructTag: `json:"original"`}
		b := &edge.Annotation{StructTag: `json:"override"`}
		result := a.Merge(b)
	...
	t.Run("merge_with_empty_struct_tag_keeps_original", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{StructTag: `json:"original"`}
		b := edge.Annotation{StructTag: ""}
		result := a.Merge(b)
		ann, ok := result.(edge.Annotation)
		require.True(t, ok)
		assert.Equal(t, `json:"original"`, ann.StructTag)
	})

	t.Run("merge_nil_pointer_keeps_original", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{StructTag: `json:"original"`}
		var b *edge.Annotation
		result := a.Merge(b)
		ann, ok := result.(edge.Annotation)
		require.True(t, ok)
		assert.Equal(t, `json:"original"`, ann.StructTag)
	...
	t.Run("merge_different_type_returns_self", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{StructTag: `json:"original"`}
		result := a.Merge(GQL{Field: "test"})
		ann, ok := result.(edge.Annotation)
		require.True(t, ok)
		assert.Equal(t, `json:"original"`, ann.StructTag)
	})

	t.Run("merge_empty_into_empty", func(t *testing.T) {
		t.Parallel()
		a := edge.Annotation{}
		b := edge.Annotation{}
		result := a.Merge(b)
