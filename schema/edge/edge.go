...
func To(name string, t any) *assocBuilder {
	typeName, err := typ(name, t)
	return &assocBuilder{desc: &Descriptor{Name: name, Type: typeName, Err: err}}
}
...
func From(name string, t any) *inverseBuilder {
	typeName, err := typ(name, t)
	return &inverseBuilder{desc: &Descriptor{Name: name, Type: typeName, Inverse: true, Err: err}}
}

// typ extracts the type name from the edge type function.
// It returns an error if the type is invalid.
func typ(edgeName string, t any) (string, error) {
	if t == nil {
		return "", fmt.Errorf("edge %q: type cannot be nil", edgeName)
	}
	rt := reflect.TypeOf(t)
	if rt == nil {
		return "", fmt.Errorf("edge %q: reflect.TypeOf returned nil", edgeName)
	}
	if rt.Kind() != reflect.Func {
		return "", fmt.Errorf("edge %q: expected func type but got %s", edgeName, rt.Kind())
	}
	if rt.NumIn() == 0 {
		return "", fmt.Errorf("edge %q: type function has no input parameters", edgeName)
	}
	return rt.In(0).Name(), nil
}
...
func (b *assocBuilder) Through(name string, t any) *assocBuilder {
	typeName, err := typ(name, t)
	if err != nil && b.desc.Err == nil {
		b.desc.Err = err
	}
	b.desc.Through = &struct{ N, T string }{N: name, T: typeName}
	return b
}
...
// Descriptor implements the velox.Descriptor interface.
