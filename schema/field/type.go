...
// IsStandardType reports if the type is a standard Go type with a well-defined zero value.
// Standard types: bool, string, int*, uint*, float*, time.Time, []byte, UUID, JSON.
// Non-standard types: enum, other (custom types).
// Note: TypeOther can still use Go's zero value (Type{}), but is not considered "standard".
func (t Type) IsStandardType() bool {
	switch t {
	case TypeBool, TypeString, TypeTime, TypeBytes, TypeUUID, TypeJSON,
		TypeInt, TypeInt8, TypeInt16, TypeInt32, TypeInt64,
		TypeUint, TypeUint8, TypeUint16, TypeUint32, TypeUint64,
		TypeFloat32, TypeFloat64:
		return true
	default:
		// TypeEnum requires explicit Default() - empty string "" is not a valid enum value
		// TypeOther can use zero value (Type{}) but is handled separately
		return false
	}
}
...
func (t Type) ConstName() string {
	...
	case int(t) < len(constNames) && constNames[t] != "":
		return constNames[t]
	default:
		return "Type" + cases.Title(language.English).String(typeNames[t])
	}
}
