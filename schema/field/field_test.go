...
func TestNullable(t *testing.T) {
	tests := []struct {
		...
		{"string", field.String("s").Nullable()},
		{"text", field.Text("t").Nullable()},
		{"int", field.Int("i").Nullable()},
		{"int8", field.Int8("i8").Nullable()},
		{"int16", field.Int16("i16").Nullable()},
		{"int32", field.Int32("i32").Nullable()},
		{"int64", field.Int64("i64").Nullable()},
		{"uint", field.Uint("u").Nullable()},
		{"uint8", field.Uint8("u8").Nullable()},
		{"uint16", field.Uint16("u16").Nullable()},
		{"uint32", field.Uint32("u32").Nullable()},
		{"uint64", field.Uint64("u64").Nullable()},
		{"float", field.Float("f").Nullable()},
		{"float32", field.Float32("f32").Nullable()},
		{"bool", field.Bool("b").Nullable()},
		{"time", field.Time("tm").Nullable()},
		{"enum", field.Enum("e").Values("a", "b").Nullable()},
		{"uuid", field.UUID("u", uuid.UUID{}).Nullable()},
		{"bytes", field.Bytes("by").Nullable()},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			d := tt.f.Descriptor()
			assert.True(t, d.Optional, "Nullable should set Optional")
			assert.True(t, d.Nillable, "Nullable should set Nillable")
		})
	}
}
