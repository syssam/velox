...
// Validators are executed in the order they are defined on the field builder.
// For example, field.String("name").NotEmpty().MaxLen(100) runs NotEmpty first,
// then MaxLen. If any validator fails, subsequent validators are still executed
...
func (b *stringBuilder) MinLen(i int) *stringBuilder {
	b.desc.Validators = append(b.desc.Validators, func(v string) error {
		if len(v) < i {
			return fmt.Errorf("value length %d is less than minimum %d", len(v), i)
		}
		return nil
	})
	return b
...
func (b *stringBuilder) MinRuneLen(i int) *stringBuilder {
	b.desc.Validators = append(b.desc.Validators, func(v string) error {
		if utf8.RuneCountInString(v) < i {
			return fmt.Errorf("value rune length %d is less than minimum %d", utf8.RuneCountInString(v), i)
		}
		return nil
	})
	return b
...
func (b *stringBuilder) MaxLen(i int) *stringBuilder {
	...
	b.desc.Validators = append(b.desc.Validators, func(v string) error {
		if len(v) > i {
			return fmt.Errorf("value length %d exceeds maximum %d", len(v), i)
		}
		return nil
	})
	return b
...
func (b *stringBuilder) MaxRuneLen(i int) *stringBuilder {
	...
	b.desc.Validators = append(b.desc.Validators, func(v string) error {
		if utf8.RuneCountInString(v) > i {
			return fmt.Errorf("value rune length %d exceeds maximum %d", utf8.RuneCountInString(v), i)
		}
		return nil
	})
	return b
...
func (b *stringBuilder) DefaultFunc(fn any) *stringBuilder {
	t := reflect.TypeOf(fn)
	if t == nil {
		b.desc.Err = fmt.Errorf("field.String(%q).DefaultFunc expects func but got nil", b.desc.Name)
		return b
	}
...
// Nullable makes the field NULL in the database and *T (pointer) in Go.
// Equivalent to Optional().Nillable() — use when "this field can be absent"
// should mean NULL at every layer.
...
// In SQL dialects this is the column name.
...
// Descriptor implements the velox.Field interface by returning its descriptor.
...
// SchemaType overrides the default database type with a custom
// schema type (per dialect) for bool.
//
//	field.Bool("active").
//		SchemaType(map[string]string{
//			dialect.MySQL: "tinyint(1)",
//		})
func (b *boolBuilder) SchemaType(types map[string]string) *boolBuilder {
	b.desc.SchemaType = types
	return b
}
...
func (b *bytesBuilder) DefaultFunc(fn any) *bytesBuilder {
	t := reflect.TypeOf(fn)
	if t == nil {
		b.desc.Err = fmt.Errorf("field.Bytes(%q).DefaultFunc expects func but got nil", b.desc.Name)
		return b
	}
	if t.Kind() != reflect.Func {
...
func (b *bytesBuilder) MaxLen(i int) *bytesBuilder {
	...
	b.desc.Validators = append(b.desc.Validators, func(buf []byte) error {
		if len(buf) > i {
			return fmt.Errorf("value length %d exceeds maximum %d", len(buf), i)
		}
		return nil
	})
	return b
...
func (b *bytesBuilder) MinLen(i int) *bytesBuilder {
	b.desc.Validators = append(b.desc.Validators, func(buf []byte) error {
		if len(buf) < i {
			return fmt.Errorf("value length %d is less than minimum %d", len(buf), i)
		}
		return nil
	})
	return b
...
//			if len(b) % 2 == 0 {
//				return fmt.Errorf("velox/schema: blob length is even: %d", len(b))
//			}
//			return nil
//		})
...
func (b *uuidBuilder) Default(fn any) *uuidBuilder {
	...
	if typ == nil {
		b.desc.Err = fmt.Errorf("expect type (func() %s) for uuid default value, got nil", b.desc.Info)
		return b
	}
...
// Equivalent to Optional().Nillable() — use when "this field can be absent"
// should mean NULL at every layer.
func (b *otherBuilder) Nullable() *otherBuilder {
	b.desc.Optional = true
	b.desc.Nillable = true
	return b
}
...
type Descriptor struct {
	...
	Nillable         bool                    // nullable field in database, pointer type in struct.
	Optional         bool                    // optional on create, not required in API input.
