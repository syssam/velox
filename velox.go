// Package velox provides core interfaces and types for defining entity schemas
// in the Velox ORM framework.
//
// Velox is a type-safe Go ORM framework with integrated code generation for
// GraphQL services. It generates production-ready database access code and
// GraphQL schemas/resolvers from declarative schema definitions written in Go.
//
// # Schema Definition
//
...
type (
	...
	Interface interface {
		...
		// is planned to be removed in v0.5.0. New code should use Annotations instead.
		//
...
type (
	...
	// data. The implementation is generated by velox.
	Mutation interface {
...
type (
	...
	// Calling Querier.Query(velox.Query) triggers the execution
	// of the query.
	Querier interface {
	...
	// Interceptor defines an execution middleware for various types of Velox queries.
	...
	// Traverser defines a graph-traversal middleware for various types of Velox queries.
...
// Intercept is a pass-through that returns the next Querier unchanged, allowing
// TraverseFunc to satisfy the Interceptor interface without participating in the intercept chain.
...
// WithInterceptors executes a Querier through an interceptor chain and
// returns the result cast to V. This is the shared implementation used by
// all generated query builders (both root-mode and entity-mode).
...
// WithHooks runs a mutation function through a hook chain and returns the
// result cast to V. Shared implementation for all generated mutation builders.
//
// The check function validates the mutation state (required fields, etc.)
// and runs INSIDE the hook chain (after hooks have modified the mutation),
// matching Ent's behavior where check() is inside sqlSave.
//
// The PM type parameter is a pointer-to-mutation type, enabling the innermost
// MutateFunc to copy hook-modified mutations back to the builder (Ent-style).
func WithHooks[V Value, M any, PM interface {
	*M
	Mutation
}](ctx context.Context, exec func(context.Context) (V, error), mutation PM, hooks []Hook) (v V, err error) {
	if len(hooks) == 0 {
		return exec(ctx)
	}
	var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
		...
		// Copy the (potentially hook-modified) mutation back to the builder's pointer.
		*mutation = *mutationT
		return exec(ctx)
	})
	for i := len(hooks) - 1; i >= 0; i-- {
		if hooks[i] == nil {
			return v, fmt.Errorf("velox: uninitialized hook (forgotten import runtime?)")
		}
		mut = hooks[i](mut)
	}
	...
	nv, ok := rv.(V)
	if !ok {
		return v, fmt.Errorf("unexpected node type %T returned from mutation", rv)
	}
	return nv, nil
}
...
const (
	OpQueryFirst   = "QueryFirst"
	OpQueryFirstID = "QueryFirstID"
	OpQueryOnly    = "QueryOnly"
	OpQueryOnlyID  = "QueryOnlyID"
	OpQueryAll     = "QueryAll"
	OpQueryIDs     = "QueryIDs"
	OpQueryCount   = "QueryCount"
	OpQueryExist   = "QueryExist"
	OpQueryGroupBy = "QueryGroupBy"
	OpQuerySelect  = "QuerySelect"
)
