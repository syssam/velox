...
func normalizePkg(c *gen.Config) error {
	...
	// Note: We do NOT modify the package import path - hyphens are valid in
	// import paths and must match the module name exactly. Only the package
	// identifier (used in code as `package foo`) needs to be valid.
	base := path.Base(c.Package)
	// Convert hyphens to underscores for the identifier check only
...
//
//	config, _ := gen.NewConfig(
//		gen.WithTemplates(entswag.Templates...),
//	)
//
...
func generate(schemaPath string, cfg *gen.Config) error {
	...
	// Use custom generator if configured, otherwise default to SQL dialect
	if cfg.Generator != nil {
		return cfg.Generator.Generate(graph)
	}
	return sql.Generate(graph)
}
