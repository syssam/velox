// Package internal provides internal utilities for the compiler.
package internal
...
type Snapshot struct {
	...
	Generator func(*gen.Graph) error // Generator function to use for code generation.
}
...
func (s *Snapshot) Restore() error {
	...
	if s.Generator == nil {
		return fmt.Errorf("snapshot restore requires a Generator function")
	}
	return s.Generator(graph)
}
...
func (s *Snapshot) parseSnapshot(buf []byte) (*gen.Snapshot, error) {
	...
	if err = json.Unmarshal(line, &local); err != nil {
		return nil, fmt.Errorf("unmarshal local snapshot in %s: %w", s.Path, err)
	}
	...
	if err := json.Unmarshal(line, &other); err != nil {
		return nil, fmt.Errorf("unmarshal other snapshot in %s: %w", s.Path, err)
	}
	merge(local, other)
	return local, nil
}
...
// Upstream-only annotations on existing fields/edges are discarded
// with a warning (local takes precedence).
func mergeSchema(local, other *load.Schema) {
	...
	for ant := range other.Annotations {
		if _, ok := local.Annotations[ant]; !ok {
			...
			slog.Warn("snapshot merge: discarding upstream schema annotation (local takes precedence)",
				"schema", local.Name, "annotation", ant)
		}
	}
...
// Upstream-only annotations on existing fields are discarded with a warning.
func mergeField(schemaName string, local, other *load.Field) {
	if local.Annotations == nil && other.Annotations != nil {
		local.Annotations = make(map[string]any)
	}
	for ant := range other.Annotations {
		if _, ok := local.Annotations[ant]; !ok {
			...
			slog.Warn("snapshot merge: discarding upstream field annotation (local takes precedence)",
				"schema", schemaName, "field", local.Name, "annotation", ant)
		}
	}
...
// Upstream-only annotations on existing edges are discarded with a warning.
func mergeEdge(schemaName string, local, other *load.Edge) {
	if local.Annotations == nil && other.Annotations != nil {
		local.Annotations = make(map[string]any)
	}
	for ant := range other.Annotations {
		if _, ok := local.Annotations[ant]; !ok {
			...
			slog.Warn("snapshot merge: discarding upstream edge annotation (local takes precedence)",
				"schema", schemaName, "edge", local.Name, "annotation", ant)
		}
	}
}
...
func IsBuildError(err error) bool {
	if strings.HasPrefix(err.Error(), "velox/load: #") {
		return true
	}
