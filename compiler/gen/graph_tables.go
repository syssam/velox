...
//
// NOTE: This is NOT the production migration path. The runtime migration uses the
// STATIC `Tables` slice generated by compiler/gen/sql/migrate.go (genMigrateSchema)
// into <pkg>/migrate/schema.go, which Schema.Create passes to migrate.Create. This
// method is currently used only by tests; it mirrors Ent's gen.Graph.Tables() API.
// The two FK/column builders must not drift — both read referential actions through
// the single source of truth Edge.DeleteAction (an earlier drift between them was
// the OnDelete edge-placement bug). If you change FK/migration behavior, change
// sql/migrate.go; changing only this method updates nothing that ships.
