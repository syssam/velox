...
//
// It is the one table builder: the generated migrate/schema.go renders what
// it returns (compiler/gen/sql/migrate.go), and Schema.Create migrates from
// that. Change migrations here.
func (g *Graph) Tables() (all []*schema.Table, err error) {
	...
	for _, n := range g.Nodes {
		...
		for _, e := range n.Edges {
			...
			case O2O, O2M:
				...
				if owner == nil || ref == nil {
					continue // a table skipped with sqlschema.Skip is managed elsewhere
				}
		...
		if n.HasCompositeID() && tables[n.Table()] != nil {
