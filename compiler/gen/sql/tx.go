...
func genTx(h gen.GeneratorHelper) *jen.File {
	...
	f.Func().Id("newTxWithOptions").Params(
	...
	).Params(jen.Op("*").Id("Tx"), jen.Error()).BlockFunc(func(grp *jen.Group) {
		grp.If(
		...
		grp.Add(jen.List(jen.Id("drv"), jen.Id("ok")).Op(":=").Id("c").Dot("driver").Op(".").Parens(
			jen.Interface(
		...
		grp.Add(jen.List(jen.Id("tx"), jen.Id("err")).Op(":=").Id("drv").Dot("BeginTx").Call(jen.Id("ctx"), jen.Id("opts")))
		grp.If(jen.Id("err").Op("!=").Nil()).Block(
			jen.Return(jen.Nil(), jen.Qual("fmt", "Errorf").Call(jen.Lit("starting transaction: %w"), jen.Id("err"))),
		)
		grp.Id("cfg").Op(":=").Id("c")
		grp.Id("cfg").Dot("driver").Op("=").Op("&").Id("txDriver").Values(jen.Dict{
			jen.Id("tx"):         jen.Id("tx"),
		...
		grp.Id("t").Op(":=").Op("&").Id("Tx").Values(jen.Dict{
			jen.Id("config"): jen.Id("cfg"),
			jen.Id("ctx"):    jen.Id("ctx"),
		})
		// Init entity clients on the Tx.
		grp.Id("t").Dot("init").Call()
		grp.Return(jen.Id("t"), jen.Nil())
	})
	...
	f.Func().Params(jen.Id("tx").Op("*").Id("Tx")).Id("OnCommit").Params(jen.Id("f").Id("CommitHook")).Block(
		...
		jen.If(jen.Op("!").Id("ok")).Block(
			jen.Panic(jen.Lit("velox: OnCommit called on a non-transactional Tx")),
		),
		jen.Id("txDriver").Dot("mu").Dot("Lock").Call(),
	...
	f.Func().Params(jen.Id("tx").Op("*").Id("Tx")).Id("OnRollback").Params(jen.Id("f").Id("RollbackHook")).Block(
		jen.List(jen.Id("txDriver"), jen.Id("ok")).Op(":=").Id("tx").Dot("config").Dot("driver").Op(".").Parens(jen.Op("*").Id("txDriver")),
		jen.If(jen.Op("!").Id("ok")).Block(
			jen.Panic(jen.Lit("velox: OnRollback called on a non-transactional Tx")),
		),
		jen.Id("txDriver").Dot("mu").Dot("Lock").Call(),
	...
	// Client — lazy-init a Client with the transaction's config.
	...
	f.Comment("BaseDriver returns the underlying non-transactional driver.")
	f.Comment("Implements runtime.TxDriverUnwrapper — consumed by entity Unwrap() to swap")
	f.Comment("back to the base driver after Tx.Commit/Rollback.")
	f.Func().Params(jen.Id("tx").Op("*").Id("txDriver")).Id("BaseDriver").Params().Qual(dialectPkg(), "Driver").Block(
		jen.Return(jen.Id("tx").Dot("drv")),
	)
