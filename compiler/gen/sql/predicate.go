...
func genPredicate(h gen.GeneratorHelper, t *gen.Type) *jen.File {
	...
	if h.FeatureEnabled(gen.FeatureEntPredicates.Name) {
...
func genGenericPredicate(h gen.GeneratorHelper, t *gen.Type) *jen.File {
	...
	// And/Or/Not combinators — use non-generic versions to avoid monomorphization
	genPredicateCombinators(h, f, t)
...
func genIDPredicates(h gen.GeneratorHelper, f *jen.File, t *gen.Type) {
	...
	// IDIn — uses non-generic FieldInValues to avoid monomorphization
	f.Comment("IDIn applies the In predicate on the ID field.")
	f.Func().Id("IDIn").Params(
		jen.Id("ids").Op("...").Add(h.IDType(t)),
	).Add(h.PredicateType(t)).Block(
		jen.Id("fArgs").Op(":=").Make(jen.Index().Any(), jen.Len(jen.Id("ids"))),
		jen.For(jen.Id("i").Op(":=").Range().Id("ids")).Block(
			jen.Id("fArgs").Index(jen.Id("i")).Op("=").Id("ids").Index(jen.Id("i")),
		),
		jen.Return(h.PredicateType(t)).Call(
			jen.Qual(h.SQLPkg(), "FieldInValues").Call(jen.Id(idConst), jen.Id("fArgs").Op("...")),
		),
	)

	// IDNotIn — uses non-generic FieldNotInValues to avoid monomorphization
	...
	f.Func().Id("IDNotIn").Params(
		...
		jen.Return(h.PredicateType(t)).Call(
			jen.Qual(h.SQLPkg(), "FieldNotInValues").Call(jen.Id(idConst), jen.Id("fArgs").Op("...")),
		),
	)
...
func genFieldPredicates(h gen.GeneratorHelper, f *jen.File, t *gen.Type, field *gen.Field) {
	...
	for _, op := range field.Ops() {
		...
		if op.Niladic() {
			...
			// Use non-generic FieldInValues/FieldNotInValues to avoid monomorphization.
			sqlFunc := "Field" + op.Name() + "Values"
			f.Func().Id(funcName).Params(
				jen.Id(arg).Op("...").Add(subpkgBaseType(h, field)),
			).Add(h.PredicateType(t)).Block(
				jen.Id("fArgs").Op(":=").Make(jen.Index().Any(), jen.Len(jen.Id(arg))),
				jen.For(jen.Id("i").Op(":=").Range().Id(arg)).Block(
					jen.Id("fArgs").Index(jen.Id("i")).Op("=").Id(arg).Index(jen.Id("i")),
				),
				jen.Return(h.PredicateType(t)).Call(
					jen.Qual(h.SQLPkg(), sqlFunc).Call(jen.Id(fieldConst), jen.Id("fArgs").Op("...")),
				),
			)
		} else {
...
func genEdgePredicates(h gen.GeneratorHelper, f *jen.File, t *gen.Type, edge *gen.Edge) {
	...
	schemaConfigStatements := func() []jen.Code {
		...
		stampStmts := genSchemaConfigStampStep(t, edge)
		stmts := make([]jen.Code, 0, 1+len(stampStmts))
		stmts = append(stmts, jen.Id("schemaConfig").Op(":=").Qual(h.InternalPkg(), "SchemaConfigFromContext").Call(
	...
	hasWithBody := make([]jen.Code, 0, 2+len(schemaStmts))
	hasWithBody = append(hasWithBody, jen.Id("step").Op(":=").Id("new"+structField+"Step").Call())
	hasWithBody = append(hasWithBody, schemaStmts...)
...
// genPredicateCombinators generates And, Or, Not combinators as package-level
// var declarations that delegate to the generic sql.PredicateAnd/Or/Not helpers.
func genPredicateCombinators(h gen.GeneratorHelper, f *jen.File, t *gen.Type) {
	sqlPkg := h.SQLPkg()
	predType := h.PredicateType(t)

	f.Comment("And groups predicates with the AND operator between them.")
	f.Var().Id("And").Op("=").Qual(sqlPkg, "PredicateAnd").Types(predType)

	f.Comment("Or groups predicates with the OR operator between them.")
	f.Var().Id("Or").Op("=").Qual(sqlPkg, "PredicateOr").Types(predType)

	f.Comment("Not applies the not operator on the given predicate.")
	f.Var().Id("Not").Op("=").Qual(sqlPkg, "PredicateNot").Types(predType)
}

// edgeSchemaFieldName returns the SchemaConfig field name for the edge's
// relation table. This follows Ent's logic:
//   - OwnFK (M2O, FK on this entity): use thisEntity name
//   - M2M with Through: use through entity name
//   - M2M inverse: use targetEntity + Pascal(inverse edge name)
//   - M2M owner: use thisEntity + edge struct field name
//   - Otherwise (O2M, FK on target): use targetEntity name
func edgeSchemaFieldName(t *gen.Type, edge *gen.Edge) string {
	if edge.OwnFK() {
		return t.Name
	}
	if edge.M2M() {
		...
		return t.Name + edge.StructField()
	}
	return edge.Type.Name
}

// genSchemaConfigStampStep generates jen statements to stamp step.To.Schema
// and step.Edge.Schema from a schemaConfig variable that is already in scope.
// The caller must ensure `schemaConfig` is assigned before calling this.
func genSchemaConfigStampStep(t *gen.Type, edge *gen.Edge) []jen.Code {
	return []jen.Code{
