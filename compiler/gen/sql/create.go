	...
	f.Func().Params(jen.Id(recv).Op("*").Id(builderName)).Id("check").Params().Error().BlockFunc(func(grp *jen.Group) {
		for _, fd := range fields {
			if t.HasOneFieldID() && fd.Name == t.ID.Name {
				// The ID has no "missing required field" check — it may be
				// generated by the database or by DefaultID. Its validator
				// still has to run; see genCreateCheckID below.
				continue
			}
...
// genCreateCheckID emits the validator call for a user-defined ID field.
//
// The main check() loop skips the ID (it has no required-field check — the
// database or DefaultID may supply it), which used to skip its validator too:
// `IDValidator` was declared in the leaf package and assigned by the runtime
// init, and then nothing ever called it. A schema writing
// `field.String("id").NotEmpty().MaxLen(20)` generated, compiled, and
// validated nothing. Ent calls it from create check() (see
// entc/integration/gremlin/ent/item_create.go).
//
// Mutation.ID() is typed `(any, bool)` because the interface is shared across
// entities, so the value is asserted back to the ID's Go type. SetID is
// typed, so the assertion cannot fail in practice — it returns an error
// rather than skipping, so a future change that breaks the invariant is loud.
func genCreateCheckID(h gen.GeneratorHelper, grp *jen.Group, t *gen.Type, recv string, validationErr func(name jen.Code, errVal jen.Code) jen.Code) {
	...
	grp.If(
		jen.List(jen.Id("v"), jen.Id("ok")).Op(":=").Id(recv).Dot("mutation").Dot("ID").Call(),
		jen.Id("ok"),
	).Block(
		jen.List(jen.Id("idv"), jen.Id("okv")).Op(":=").Id("v").Assert(h.BaseType(t.ID)),
		jen.If(jen.Op("!").Id("okv")).Block(
			jen.Return(validationErr(
				jen.Lit(t.ID.Name),
				jen.Qual("fmt", "Errorf").Call(
					jen.Lit("unexpected type %T for field \""+t.Name+"."+t.ID.Name+"\""), jen.Id("v"),
				),
			)),
		),
		jen.If(
			jen.Id("err").Op(":=").Qual(h.LeafPkgPath(t), "IDValidator").Call(jen.Id("idv")),
			jen.Id("err").Op("!=").Nil(),
		).Block(
			jen.Return(validationErr(
				jen.Lit(t.ID.Name),
				jen.Qual("fmt", "Errorf").Call(
					jen.Lit("validator failed for field \""+t.Name+"."+t.ID.Name+"\": %w"), jen.Id("err"),
				),
			)),
		),
	)
