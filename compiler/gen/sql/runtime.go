...
func genRuntimePolicies(h gen.GeneratorHelper, grp *jen.Group, t *gen.Type, schemaPkg, entityPkg, pkg string) {
	...
	// Register the policy VARIABLE in the runtime registry so cross-package
	// edge queries and eager loads can look it up by entity name and wire it
	// onto freshly-built target queries (e.g. entity.User.QueryPosts() wires
	// the Post policy). The address, not the value: a lookup reads what the
	// target's client would, even after RuntimePolicy is replaced.
	grp.Qual(runtimePkg, "RegisterEntityPolicy").Call(jen.Lit(t.Name), jen.Op("&").Qual(entityPkg, "RuntimePolicy"))
}
