...
func TestNewGraph(t *testing.T) {
	...
	_, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, T1)
	...
	for i, nullable := range []bool{true, true, false} {
...
func TestNewGraphRequiredLoop(t *testing.T) {
	_, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, &load.Schema{
		Name: "T1",
		Edges: []*load.Edge{
	...
	_, err = NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]},
		&load.Schema{
			Name: "User",
			Edges: []*load.Edge{
...
func TestNewGraphDuplicateEdges(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `User schema contains multiple "groups" edges`)
}
...
func TestNewGraphDuplicateEdgeField(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `User schema cannot contain field and edge with the same name "parent"`)
}
...
func TestNewGraphThroughUndefinedType(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `edge T1.groups defined with Through("groups_edge", T2.Type), but type T2 was not found`)
}
...
func TestNewGraphThroughInvalidRel(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `edge T1.groups Through("groups_edge", T2.Type) is allowed only on M2M edges, but got: "O2O"`)
}
...
func TestNewGraphThroughDuplicates(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `edge User.groups defined with Through("group_edges", T1.Type), but schema User already has an edge named group_edges`)
}
...
func TestFKColumns(t *testing.T) {
	...
	require := require.New(t)
	graph, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user, &load.Schema{Name: "Pet"})
	require.NoError(err)
	t1 := graph.Nodes[0]
	...
	graph, err = NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user,
		&load.Schema{
			Name: "Pet",
			Edges: []*load.Edge{
...
func TestAbortDuplicateFK(t *testing.T) {
	...
	g, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user, pet, car)
...
func TestPosition(t *testing.T) {
	antFn := func(s string) map[string]any {
		return map[string]any{sqlschema.Annotation{}.Name(): map[string]string{"schema": s}}
	}
	var (
		user = &load.Schema{
			Name: "User",
	...
	g, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user, pet, petView, car, carOwner)
	require.NoError(t, err)
	ts, err := g.Tables()
	require.NoError(t, err)
	require.Len(t, ts, 5)
...
func TestEnsureCorrectFK(t *testing.T) {
	...
	_, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user, pet)
	require.Error(t, err)
	require.Contains(t, err.Error(), `column "owner_id" definition on edge "pets" should be replaced with Field("owner_id") on its reference "owner"`)
...
func TestGraph_Gen(t *testing.T) {
	...
	for _, name := range []string{"velox", "client"} {
		_, err = os.Stat(fmt.Sprintf("%s/%s.go", target, name))
		require.NoError(err)
	}
	// Ensure entity files were generated (flat layout from testGenerator).
	...
	require.NotNil(graph)
	require.NoError(graph.Gen())
	// Ensure t1 entity files exist (flat layout from testGenerator).
	for _, suffix := range []string{"", "_create", "_update", "_delete", "_query"} {
		_, err := os.Stat(filepath.Join(target, "t1"+suffix+".go"))
		require.NoError(err, "t1%s.go should exist", suffix)
	}
}
...
func TestEdgeFieldCollation(t *testing.T) {
	var (
		user = &load.Schema{
			Name: "User",
			Fields: []*load.Field{
				{Name: "id", Info: &field.TypeInfo{Type: field.TypeString}, Annotations: dict("sql", dict("collation", "utf8mb4_bin"))},
			},
		}
		post = &load.Schema{
			Name: "Post",
			Fields: []*load.Field{
				{Name: "author_id", Info: &field.TypeInfo{Type: field.TypeString}, Annotations: dict("sql", dict("collation", "utf8mb4_bin"))},
			},
			Edges: []*load.Edge{
	...
	g, err := NewGraph(&Config{Package: "entc/gen", Storage: drivers["sql"]}, user, post)
	require.NoError(t, err)
	tables, err := g.Tables()
	require.NoError(t, err)
