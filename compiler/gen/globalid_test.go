...
func TestIncrementStartAnnotation(t *testing.T) {
	...
	a := &sqlschema.Annotation{IncrementStart: p(100)}
	aMap := must(gen.ToMap(a))
	s := []*load.Schema{
		{
			Name:        "T1",
			Annotations: map[string]any{a.Name(): aMap},
		},
	}
	c := &gen.Config{
		Package: "entc/gen",
	...
	require.Nil(t, g)
	a = &sqlschema.Annotation{IncrementStart: p(1 << 32)}
	...
	a3Map := must(gen.ToMap(a))
	s = append(s, &load.Schema{Name: "T2"}, &load.Schema{
		Name:        "T3",
		Annotations: map[string]any{a.Name(): a3Map},
	})
	g, err = gen.NewGraph(c, s...)
...
`,
	...
	require.NoError(t, os.WriteFile(p, []byte(cflct), 0o644))
