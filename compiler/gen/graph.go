...
// This uses templates for code generation with parallel execution
// and optimized formatting (go/format + goimports library).
func generate(g *Graph) error {
	// Use template-based generation for core files
	if err := GenerateTemplates(g); err != nil {
		return fmt.Errorf("template generation: %w", err)
	}

	// Track generated assets for cleanup
	var assets assets
	for _, n := range g.Nodes {
		assets.addDir(filepath.Join(g.Config.Target, n.PackageDir()))
	}

	// Handle external templates for backward compatibility
	tpls, external := g.templates()
	if len(external) > 0 {
		for _, tmpl := range external {
			...
			if err := tpls.ExecuteTemplate(b, tmpl.Name, g); err != nil {
				return fmt.Errorf("execute template %q: %w", tmpl.Name, err)
			}
			assets.add(filepath.Join(g.Config.Target, tmpl.Format), b.Bytes())
		}
		// Write external template assets
		if err := assets.write(); err != nil {
			return err
		}
		// Format external templates only (Jennifer output is already formatted)
		if err := assets.format(); err != nil {
			return err
		}
	}

	// Handle feature cleanup
	...
	// Cleanup nodes' assets and old template files that are no longer needed
	...
	for _, n := range deletedTemplates {
		if err := os.Remove(filepath.Join(g.Target, n)); err != nil && !os.IsNotExist(err) {
			slog.Warn("failed to remove old file", "path", filepath.Join(g.Target, n), "error", err)
		}
	}
...
func (g *Graph) addNode(schema *load.Schema) error {
	t, err := NewType(g.Config, schema)
	if err != nil {
		return fmt.Errorf("create type %s: %w", schema.Name, err)
	}
	...
	for _, e := range schema.Edges {
		...
		if _, ok = t.fields[e.Name]; ok {
			return &EdgeError{
				From:    schema.Name,
				Edge:    e.Name,
				Message: fmt.Sprintf("%s schema cannot contain field and edge with the same name %q", schema.Name, e.Name),
			}
		}
		if _, ok = seen[e.Name]; ok {
			return &EdgeError{
				From:    schema.Name,
				Edge:    e.Name,
				Message: fmt.Sprintf("%s schema contains multiple %q edges", schema.Name, e.Name),
			}
		}
		seen[e.Name] = struct{}{}
		switch {
...
func (g *Graph) resolve(t *Type) error {
	for _, e := range t.Edges {
		...
		case e.IsInverse():
			...
			e.Ref, ref.Ref = ref, e
			table := t.Table()
			// Name the foreign-key column. Prefer explicit FK field from edge.Field() if defined,
			// otherwise generate in a format: "<Edge-Owner>_<Edge-Name>".
			var column string
			if fld := e.Field(); fld != nil {
				// Use the explicit FK field name (e.g., "journal_entry_id" from edge.Field("journal_entry_id"))
				column = fld.StorageKey()
			} else {
				// Generate column name from edge names
	...
	for _, typ := range deleted {
		for _, t := range Templates {
			...
			if err != nil && !os.IsNotExist(err) {
				slog.Warn("failed to remove old file", "path", filepath.Join(target, t.Format(typ)), "error", err)
			}
		}
		err := os.Remove(filepath.Join(target, typ.PackageDir()))
		if err != nil && !os.IsNotExist(err) {
			slog.Warn("failed to remove old directory", "path", filepath.Join(target, typ.PackageDir()), "error", err)
		}
	}
}
