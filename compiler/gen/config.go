...
// accepting a Feature value directly instead of a string key.
func (c *Config) FeatureEnabledF(f Feature) bool {
	return c.featureEnabled(f)
}

// featureEnabled reports if the given feature-flag is enabled.
// A feature is enabled if it is explicitly listed in Config.Features,
// or if its Default field is true and it has not been explicitly disabled.
