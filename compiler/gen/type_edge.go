...
// DefiningType returns the type that declares this edge in its schema.
// Unlike Owner (which for inverse edges points to the assoc type),
// DefiningType always returns the type where the edge appears in Edges().
func (e Edge) DefiningType() *Type {
	if e.IsInverse() && e.Ref != nil {
		return e.Ref.Type
	}
	return e.Owner
}
...
// SetDef sets the edge definition. Used by tests to configure edge properties
// that are normally set during schema loading (e.g., Field, StorageKey).
func (e *Edge) SetDef(d *load.Edge) { e.def = d }

// DefinedField returns the field name from .Field("xxx") on the edge definition.
func (e Edge) DefinedField() string {
	if e.def != nil {
		return e.def.Field
	}
	return ""
...
// SetForeignKey sets the foreign-key for this relation.
// Used by tests to wire up FK associations without going through setupFKs.
func (r *Relation) SetForeignKey(fk *ForeignKey) { r.fk = fk }

// Column returns the first element from the columns slice.
func (r Relation) Column() string {
	if len(r.Columns) == 0 {
		slog.Error("missing column for Relation - ensure edge has proper FK column defined",
			"table", r.Table, "type", r.Type)
		return ""
	}
	return r.Columns[0]
}
