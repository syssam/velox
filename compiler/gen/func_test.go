...
func TestExtend(t *testing.T) {
	t.Run("extends Type", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, "User", scope.Name)
...
func TestJSONString(t *testing.T) {
	...
	t.Run("marshal error is propagated", func(t *testing.T) {
		_, err := jsonString(math.Inf(1))
		assert.Error(t, err, "jsonString should propagate json.Marshal error")
	})
}
...
func TestPascalExported(t *testing.T) {
	// Test the exported Pascal function (wrapper around pascal)
	tests := []struct {
		input    string
		expected string
	}{
...
func TestHasTemplate(t *testing.T) {
	...
	t.Run("returns false for nonexistent template", func(t *testing.T) {
		assert.False(t, hasTemplate("nonexistent_template_name_xyz"))
	})

	t.Run("returns true for existing template", func(t *testing.T) {
		// The "templates" root template should exist after init
		found := false
		for _, tmpl := range templates.Templates() {
			if hasTemplate(tmpl.Name()) {
				found = true
				break
			}
		}
		assert.True(t, found, "should find at least one template")
	})
}

func TestToStringWithErrorAndStringer(t *testing.T) {
	...
	t.Run("stringer type", func(t *testing.T) {
		// field.TypeString implements fmt.Stringer
		result := toString(field.TypeString)
	...
	t.Run("int value fallback", func(t *testing.T) {
		result := toString(42)
		assert.Equal(t, "42", result)
	})

	t.Run("float value fallback", func(t *testing.T) {
