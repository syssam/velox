...
func WithIDType(t string) Option {
	return func(c *Config) error {
		...
		default:
			return NewConfigError("IDType", t, "unsupported ID type; use int, int64, uint64, string, or uuid", nil)
		}
		c.IDType = info
		return nil
	}
...
func WithIDTypeInfo(info *field.TypeInfo) Option {
	return func(c *Config) error {
		if info == nil {
			return NewConfigError("IDType", nil, "TypeInfo cannot be nil", nil)
		}
		c.IDType = info
		return nil
	}
...
func WithPackage(pkg string) Option {
	return func(c *Config) error {
		if pkg == "" {
			return NewConfigError("Package", nil, "package cannot be empty", nil)
		}
		c.Package = pkg
		return nil
	}
...
func WithSchema(schema string) Option {
	return func(c *Config) error {
		if schema == "" {
			return NewConfigError("Schema", nil, "schema cannot be empty", nil)
		}
		c.Schema = schema
		return nil
	}
...
func WithTarget(dir string) Option {
	return func(c *Config) error {
		if dir == "" {
			return NewConfigError("Target", nil, "target directory cannot be empty", nil)
		}
		c.Target = dir
		return nil
	}
...
// Dependencies declared in Feature.Requires are auto-enabled with a warning.
func WithFeatures(features ...Feature) Option {
	return func(c *Config) error {
		for _, f := range features {
			for _, req := range f.Requires {
				if dep, ok := featureByName(req); ok && !c.featureEnabled(dep) {
					slog.Warn("auto-enabling required feature dependency",
						"feature", dep.Name,
						"required_by", f.Name,
					)
					c.Features = append(c.Features, dep)
				}
			}
		}
...
func WithStorageDriver(driver string) Option {
	return func(c *Config) error {
		...
		case "sqlite", "mysql", "postgres":
			// Set SQL storage with the specified driver dialect.
			// The actual Storage is always "sql" — the driver name
			// determines which dialect is used at runtime.
			...
			return nil
		default:
			return NewConfigError("StorageDriver", driver, "unsupported driver; use sqlite, mysql, or postgres", nil)
		}
	}
}
...
func WithGenerator(g Generator) Option {
	return func(c *Config) error {
		if g == nil {
			return NewConfigError("Generator", nil, "generator cannot be nil", nil)
		}
		c.Generator = g
		return nil
	}
