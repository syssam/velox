...
type (
	...
	Edge struct {
		...
		Ref *Edge
		// Owner holds the type of the edge-owner used for FK column naming and label computation.
		// For assoc edges (edge.To), Owner is the type that declares the edge.
		// For inverse edges (edge.From), Owner is the assoc type (the other side),
		// NOT the type that declares the inverse. This asymmetry is intentional:
		// it ensures Label() and FK column names are derived from the assoc side.
		Owner *Type
...
// Receiver returns the receiver name of this node. Returns "m" (for mutation)
// as a fixed convention for generated code uniformity — all generated mutation
// methods use the same receiver name regardless of entity type.
...
func (t Type) HasValidators() bool {
	if t.HasOneFieldID() && t.ID.UserDefined && t.ID.Validators > 0 {
		return true
	}
...
func (t Type) HasDefault() bool {
	if t.HasOneFieldID() && t.ID.UserDefined && t.ID.Default {
		return true
	}
	for _, f := range t.Fields {
		if f.Default {
...
func (t Type) MutableFields() []*Field {
	...
	for _, f := range t.Fields {
		...
		if e, err := f.Edge(); err == nil && e != nil && e.Immutable {
			continue
		}
...
// CreateInputName returns the name of the create input struct (e.g., "CreateUserInput").
func (t Type) CreateInputName() string {
	return "Create" + pascal(t.Name) + "Input"
}

// UpdateInputName returns the name of the update input struct (e.g., "UpdateUserInput").
func (t Type) UpdateInputName() string {
	return "Update" + pascal(t.Name) + "Input"
}

// NewCreateInputFunc returns the constructor name (e.g., "NewCreateInput").
func (t Type) NewCreateInputFunc() string {
	return "NewCreateInput"
}

// NewUpdateInputFunc returns the constructor name (e.g., "NewUpdateInput").
func (t Type) NewUpdateInputFunc() string {
...
// ConfigMethodName returns the name of the Config getter method for this type.
// If the entity has a field named "config" or "Config", returns "RuntimeConfig" to avoid clash.
func (t Type) ConfigMethodName() string {
	if t.fields["Config"] == nil && t.fields["config"] == nil {
		return "Config"
	}
	return "RuntimeConfig"
}

// SetConfigMethodName returns the name of the SetConfig setter method for this type.
// If the entity has a field named "config" or "Config", returns "SetRuntimeConfig" to avoid clash.
func (t Type) SetConfigMethodName() string {
	if t.fields["Config"] == nil && t.fields["config"] == nil {
		return "SetConfig"
	}
