...
// TestType_ConcurrentFieldMethods verifies that HasValidators, HasDefault,
// NeedsDefaults, and MixedInFields are safe for concurrent use (race-free).
// Run with: go test -race ./compiler/gen/...
func TestType_ConcurrentFieldMethods(t *testing.T) {
	typ := &Type{
		ID: &Field{
			Name:        "id",
			UserDefined: true,
			...
			Position:    &load.Position{MixedIn: true, MixinIndex: 0},
		},
		Fields: []*Field{
			{Name: "name", Type: &field.TypeInfo{Type: field.TypeString}, Validators: 1, Default: true, Position: &load.Position{MixedIn: true, MixinIndex: 1}},
			{Name: "age", Type: &field.TypeInfo{Type: field.TypeInt}, Default: true},
		},
	}
...
// TestMutableFields_NilEdgeReturn verifies MutableFields does not panic
// when f.Edge() returns (nil, nil) for O2M assoc edges without an inverse.
func TestMutableFields_NilEdgeReturn(t *testing.T) {
	assocEdge := &Edge{
		Name: "children",
		Rel:  Relation{Type: O2M, Columns: []string{"parent_id"}},
	}
	fk := &ForeignKey{
		Edge: assocEdge,
		Field: &Field{
	...
	typ := &Type{
		...
		Fields: []*Field{
			{Name: "parent_id", Type: &field.TypeInfo{Type: field.TypeInt}, fk: fk},
			{Name: "name", Type: &field.TypeInfo{Type: field.TypeString}},
		},
	}
	assert.NotPanics(t, func() {
		fields := typ.MutableFields()
		assert.Len(t, fields, 2)
	})
}

// TestFieldBy_NilID verifies FieldBy does not panic when t.ID is nil (view types).
func TestFieldBy_NilID(t *testing.T) {
	typ := &Type{
		ID: nil,
		Fields: []*Field{
			{Name: "name", Type: &field.TypeInfo{Type: field.TypeString}},
	...
	assert.NotPanics(t, func() {
		f, ok := typ.FieldBy(func(f *Field) bool { return f.Name == "name" })
		assert.True(t, ok)
		assert.Equal(t, "name", f.Name)
	})
	assert.NotPanics(t, func() {
		_, ok := typ.FieldBy(func(f *Field) bool { return f.Name == "missing" })
		assert.False(t, ok)
	})
}

// TestMutationGet_NilTyp verifies MutationGet does not panic when f.typ is nil.
func TestMutationGet_NilTyp(t *testing.T) {
	f := Field{Name: "set_id", typ: nil}
	assert.NotPanics(t, func() {
		name := f.MutationGet()
		assert.Equal(t, "SetID", name)
	})
}

// TestMutationGet_NilTypID verifies MutationGet does not panic when f.typ.ID is nil.
func TestMutationGet_NilTypID(t *testing.T) {
	f := Field{Name: "set_id", typ: &Type{ID: nil}}
	assert.NotPanics(t, func() {
		name := f.MutationGet()
		assert.Equal(t, "SetID", name)
	})
...
// TestSqlComment_NilTyp verifies Field.sqlComment does not panic when f.typ is nil.
func TestSqlComment_NilTyp(t *testing.T) {
	f := Field{Name: "test", typ: nil}
	assert.NotPanics(t, func() {
		result := f.sqlComment()
