...
func TestConfigFeatureEnabled_WhereInputAll(t *testing.T) {
	cfg := &Config{
		Features: []Feature{FeatureWhereInputAll},
	}
	enabled, err := cfg.FeatureEnabled("graphql/whereinputall")
	if err != nil {
		t.Fatalf("FeatureEnabled returned error: %v", err)
	}
	if !enabled {
		t.Error("FeatureWhereInputAll should be enabled")
	}
}
