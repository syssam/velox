...
func (f Field) DefaultFunc() bool {
	...
	return f.def.DefaultKind == reflect.Func
}
...
// For edge-fields (foreign keys), it delegates to the owning edge's BuilderField.
func (f Field) BuilderField() string {
	if f.IsEdgeField() {
		// Edge() only errors when !IsEdgeField(), which is already guarded above.
		e, _ := f.Edge()
...
func (f Field) MutationGet() string {
	name := pascal(f.Name)
	if mutMethods[name] || (name == "SetID" && f.typ != nil && f.typ.ID != nil && f.typ.ID.UserDefined) {
		name = "Get" + name
	}
	return name
}
...
// SetForeignKey sets the foreign-key association for this field.
// Used for testing field-backed edge scenarios from external packages.
func (f *Field) SetForeignKey(fk *ForeignKey) { f.fk = fk }
...
func (f Field) NillableValue() bool {
	return f.Nillable && (f.Type.RType == nil || !f.Type.RType.IsPtr())
}
...
func (f Field) Column() *schema.Column {
	// JSON fields (including slice/map types from field.Strings/Ints/JSON) have no
	// natural zero value to write into a NOT NULL column on insert, so an Optional
	// JSON field must produce a nullable DB column. Other types only become
	// nullable via the explicit Nillable() builder.
	nullable := f.Nillable || (f.Optional && f.IsJSON())
	c := &schema.Column{
		Name:     f.StorageKey(),
		Type:     f.Type.Type,
		Unique:   f.Unique,
