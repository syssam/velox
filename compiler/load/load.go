...
type (
	// A SchemaSpec holds a serializable version of an velox.Schema
...
func (c *Config) Load() (*SchemaSpec, error) {
	...
	if err = os.MkdirAll(".velox", os.ModePerm); err != nil {
		return nil, err
	}
	target := fmt.Sprintf(".velox/%s.go", filename(spec.PkgPath))
	if err = os.WriteFile(target, buf, 0o644); err != nil {
		return nil, fmt.Errorf("velox/load: write file %s: %w", target, err)
	}
	defer os.RemoveAll(".velox")
...
var entInterface = reflect.TypeOf(struct{ velox.Interface }{}).Field(0).Type
...
func gorun(target string, buildFlags []string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("velox/load: %s", err)
	}
	return s, nil
}
