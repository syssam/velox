...
func (g *Generator) genScalars() *jen.File {
	...
	// Generate marshalers for typed JSON scalars (sorted for deterministic output)
	scalarsMap := g.typedJSONScalars
	scalarKeys := make([]string, 0, len(scalarsMap))
	for k := range scalarsMap {
		scalarKeys = append(scalarKeys, k)
	}
	slices.Sort(scalarKeys)
	for _, k := range scalarKeys {
		g.genTypedJSONMarshalers(f, scalarsMap[k])
	}
		...
		jen.Return(
			jen.Qual("github.com/99designs/gqlgen/graphql", "WriterFunc").Call(
				jen.Func().Params(jen.Id("w").Qual("io", "Writer")).Block(
					jen.List(jen.Id("data"), jen.Id("err")).Op(":=").Qual("encoding/json", "Marshal").Call(jen.Id("v")),
					jen.If(jen.Id("err").Op("!=").Nil()).Block(
						jen.Qual("io", "WriteString").Call(jen.Id("w"), jen.Lit("null")),
						jen.Return(),
					),
