...
func TestSkipSensitiveField_ReadSurfacesOnly(t *testing.T) {
	tests := []struct {
		...
		{"excluded from output type", SkipType, true},
		{"excluded from order by", SkipOrderField, true},
		{"kept on create input", SkipMutationCreateInput, false},
		{"kept on update input", SkipMutationUpdateInput, false},
		{"kept on enum type", SkipEnumField, false},
	}
