...
func TestGenerator_HelpersFilterFunctions(t *testing.T) {
	...
	t.Run("FilterNodesNoSkip", func(t *testing.T) {
		...
		nodes := gen.filterNodes(g.Nodes, graphql.SkipType)
		if len(nodes) != 2 {
	...
	t.Run("FilterNodesWithSkipMask", func(t *testing.T) {
		...
		nodes := gen.filterNodes(g.Nodes, graphql.SkipType|graphql.SkipWhereInput)
			...
			Fields: []*entgen.Field{
				{
					...
					Annotations: map[string]any{
						graphql.AnnotationName: &graphql.Annotation{
							WhereOps:    graphql.OpsEquality | graphql.OpEqualFold,
							HasWhereOps: true,
						},
					},
				},
		...
		orderType := &entgen.Type{
			...
			Fields: []*entgen.Field{
				...
				},
				{
					...
					Annotations: map[string]any{
						graphql.AnnotationName: &graphql.Annotation{
							WhereOps:    graphql.OpsComparison,
							HasWhereOps: true,
						},
					},
				},
			...
			Fields: []*entgen.Field{
				{
					...
					Type: &field.TypeInfo{Type: field.TypeString},
					Annotations: map[string]any{
						graphql.AnnotationName: &graphql.Annotation{WithWhereInputs: &enableTrue},
					},
				},
...
// TestGenerator_WhereOnEdgeConnection_ForceResolverCoupling was deleted in
// Plan 3 (2026-04-25). It pinned the obsolete @goField(forceResolver: true)
// emission on edge-with-where SDL fields. Phase B made the entity method
// accept `where *filter.XxxWhereInput` directly, so gqlgen autobinds without
// the directive. The inverse invariant (no forceResolver on edge fields) is
// pinned by TestEdgeWhere_NoForceResolverDirective in golden_test.go.
