...
func TestExtractGraphQLAnnotation_JSONFallback(t *testing.T) {
	...
	ann := map[string]any{
		"graphql": map[string]any{
			...
			"Skip":            float64(graphql.SkipWhereInput), // JSON numbers are float64
		},
	}
	...
	if result.Skip != graphql.SkipWhereInput {
		t.Errorf("Skip = %d, want %d", result.Skip, graphql.SkipWhereInput)
	}
}
...
func TestExtractGraphQLAnnotation_PointerType(t *testing.T) {
	ann := &graphql.Annotation{Type: "Member", Skip: graphql.SkipType}
	result := extractGraphQLAnnotation(map[string]any{"graphql": ann})
	if result.Type != "Member" || result.Skip != graphql.SkipType {
...
func TestBuildGoFieldDirective_BugClasses(t *testing.T) {
	...
	t.Run("rename_to_unrelated_camel_emits_directive", func(t *testing.T) {
		...
		f := &entgen.Field{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: graphql.Annotation{FieldName: "authorId"},
			},
		}
	...
	t.Run("explicit_fieldname_matching_default_camel_emits_nothing", func(t *testing.T) {
		...
		f := &entgen.Field{
			...
			Type: &field.TypeInfo{Type: field.TypeInt64},
			Annotations: map[string]any{
				graphql.AnnotationName: graphql.Annotation{FieldName: "userId"},
			},
		}
		got := g.buildGoFieldDirective(f)
		assert.Empty(t, got,
	...
	t.Run("rename_preserving_struct_field_emits_directive", func(t *testing.T) {
		...
		f := &entgen.Field{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: graphql.Annotation{FieldName: "userEmail"},
			},
		}
		got := g.buildGoFieldDirective(f)
...
func TestGenerator_ShouldSkipEdgeFKField(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			annotation: graphql.Annotation{Type: "ID"},
			wantSkip:   false,
		},
		{
			name:       "FK field with explicit FieldName is not skipped",
			annotation: graphql.Annotation{FieldName: "authorID"},
			wantSkip:   false,
		},
	}
		...
		Fields: []*entgen.Field{
			{
				...
				Annotations: map[string]any{
					"graphql": graphql.Annotation{Type: "DepreciationEntryMethod"},
				},
			},
		},
	}
