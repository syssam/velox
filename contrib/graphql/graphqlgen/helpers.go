...
func (g *Generator) filterNodes(nodes []*gen.Type, skip graphql.SkipMode) []*gen.Type {
...
// Equivalent to Ent's filterEdges function - excludes edges to composite ID
// types, and edges whose own or target type's annotation has SkipType.
func (g *Generator) filterEdges(edges []*gen.Edge) []*gen.Edge {
	...
	for _, e := range edges {
		...
		if !skipE.Is(graphql.SkipType) && !skipT.Is(graphql.SkipType) {
...
func (g *Generator) filterFields(fields []*gen.Field, skip graphql.SkipMode) []*gen.Field {
	...
	for _, f := range fields {
		...
		if annSkip.Is(skip) || annSkip.Is(graphql.SkipType) {
			continue
		}
...
func skipSensitiveRead(sensitive bool, skip graphql.SkipMode) bool {
	return sensitive && (skip == graphql.SkipType || skip == graphql.SkipOrderField)
}

// annotationSkipMode returns the SkipMode from an Annotation.
func (g *Generator) annotationSkipMode(ann graphql.Annotation) graphql.SkipMode {
	return ann.Skip
}
...
func (g *Generator) hasWhereInput(e *gen.Edge) bool {
	...
	if skipEdge.Is(graphql.SkipWhereInput) || skipType.Is(graphql.SkipWhereInput) {
		return false
	}
	// Also check that the target entity has filterable content.
	return g.hasFilterableContent(e.Type)
}
...
func (g *Generator) hasOrderField(e *gen.Edge) bool {
	...
	skipEdge := g.annotationSkipMode(antEdge)
	skipType := g.annotationSkipMode(antType)
	return !skipEdge.Is(graphql.SkipOrderField) && !skipType.Is(graphql.SkipOrderField)
}
...
func (g *Generator) nodeImplementors(t *gen.Type) []string {
	...
	if g.config.RelaySpec && !annSkip.Is(graphql.SkipType) && !slices.Contains(ann.GetImplements(), "Node") {
...
func (m *MutationDescriptor) Builders() []string {
	if m.IsCreate {
		return []string{m.Name + "Create"}
	}
	return []string{m.Name + "Update", m.Name + "UpdateOne"}
}
...
func (m *MutationDescriptor) InputFields(g *Generator) []*InputFieldDescriptor {
	...
	for _, f := range m.Fields {
		...
		if m.IsCreate && annSkip.Is(graphql.SkipMutationCreateInput) {
			continue
		}
		if !m.IsCreate && (f.Immutable || annSkip.Is(graphql.SkipMutationUpdateInput)) {
			continue
		}
...
func (m *MutationDescriptor) InputEdges(g *Generator) []*gen.Edge {
	...
	for _, e := range m.Edges {
		...
		if !m.IsCreate && (e.Immutable || annSkip.Is(graphql.SkipMutationUpdateInput)) {
			continue
		}
...
// appendUnique appends items to base, skipping any that already exist.
