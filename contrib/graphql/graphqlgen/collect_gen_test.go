...
func TestGenEntityCollection_EmitsCollectedFor(t *testing.T) {
	...
	typ := graph.Nodes[0]
	ann := func(a graphql.Annotation) map[string]any { return map[string]any{graphql.AnnotationName: a} }
	typ.Fields = append(typ.Fields,
		&gen.Field{Name: "first_name", Type: &field.TypeInfo{Type: field.TypeString},
			Annotations: ann(graphql.Annotation{CollectedFor: []string{"fullName", "initials"}})},
		&gen.Field{Name: "last_name", Type: &field.TypeInfo{Type: field.TypeString},
			Annotations: ann(graphql.Annotation{CollectedFor: []string{"fullName"}, Skip: graphql.SkipType})},
	)
...
func TestMarkCollectFields(t *testing.T) {
	graph := mockGraph()
	skipped := &gen.Type{Name: "Hidden", ID: graph.Nodes[0].ID, Annotations: map[string]any{graphql.AnnotationName: graphql.Annotation{Skip: graphql.SkipType}}}
	...
	for _, n := range graph.Nodes {
		m, _ := n.Annotations[graphql.AnnotationName].(map[string]any)
...
func TestGenEntityCollection_EmitsMapLoadsAndReads(t *testing.T) {
	...
	typ.Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{ResolverMappings: []graphql.ResolverMapping{
		graphql.Map("postCount", "Int!").Loads("posts"),
		graphql.Map("greeting", "String!").Reads("name", "email").Loads("posts"),
		graphql.Map("plain", "String!"),
	}}}
...
func TestValidateResolverMappings_LoadsAndReads(t *testing.T) {
	for name, tc := range map[string]struct {
		rm   graphql.ResolverMapping
		want string
	}{
		"unknown edge":  {graphql.Map("x", "Int!").Loads("comments"), `Map("x").Loads("comments"): User has no edge "comments"`},
		"unknown field": {graphql.Map("x", "Int!").Reads("nickname"), `Map("x").Reads("nickname"): User has no field "nickname"`},
	} {
		t.Run(name, func(t *testing.T) {
			graph := mockGraph()
			g := NewGenerator(graph, Config{ORMPackage: "example.com/app/velox", Package: "velox"})
			typ := graph.Nodes[0]
			typ.Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{ResolverMappings: []graphql.ResolverMapping{tc.rm}}}
	...
	t.Run("edge hidden from the GraphQL type", func(t *testing.T) {
		...
		g := NewGenerator(graph, Config{ORMPackage: "example.com/app/velox", Package: "velox"})
		typ := graph.Nodes[0]
		typ.Edges[0].Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{Skip: graphql.SkipType}}
		typ.Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{ResolverMappings: []graphql.ResolverMapping{graphql.Map("x", "Int!").Loads("posts")}}}
		err := g.validateResolverMappings(typ)
		require.Error(t, err)
