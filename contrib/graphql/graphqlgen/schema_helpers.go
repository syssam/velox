...
func extractGraphQLAnnotation(annotations map[string]any) graphql.Annotation {
	if annotations == nil {
		return graphql.Annotation{}
	}
	ann, ok := annotations[graphql.AnnotationName]
	if !ok {
		return graphql.Annotation{}
	}
	// Direct type assertion
	if a, ok := ann.(graphql.Annotation); ok {
		return a
	}
	// Pointer type assertion
	if a, ok := ann.(*graphql.Annotation); ok && a != nil {
		return *a
	}
...
func (g *Generator) validateResolverMappings(t *gen.Type) error {
	...
	for _, rm := range ann.ResolverMappings {
		...
		for _, name := range rm.LoadEdges {
			...
			if findEdge(&gen.Type{Edges: g.filterEdges(t.Edges)}, name) == nil {
...
func (g *Generator) getTypeAnnotation(t *gen.Type) graphql.Annotation {
	return extractGraphQLAnnotation(t.Annotations)
}

func (g *Generator) getFieldAnnotation(f *gen.Field) graphql.Annotation {
	return extractGraphQLAnnotation(f.Annotations)
}

func (g *Generator) getEdgeAnnotation(e *gen.Edge) graphql.Annotation {
...
func renderDirectives(dirs []graphql.Directive) string {
	if len(dirs) == 0 {
		return ""
	}
...
func (g *Generator) collectSubscriptions() []graphql.SubscriptionConfig {
	var subs []graphql.SubscriptionConfig
	for _, t := range g.graph.Nodes {
		ann := g.getTypeAnnotation(t)
