...
// Use this when calling graphqlgen.Generate() directly instead of using hooks.
		...
		return gen.GenerateFunc(func(g *gen.Graph) error {
				...
				for _, t := range g.Nodes {
					...
					if err == nil {
						...
						if err := json.Unmarshal(data, &m); err == nil {
							t.Annotations[graphql.AnnotationName] = m
						}
					}
				}
			}
			...
			for _, t := range g.Nodes {
				for _, f := range t.DeprecatedFields() {
					...
					if !hasDeprecated {
						ann.Directives = append(ann.Directives, graphql.Deprecated(f.DeprecationReason()))
						if f.Annotations == nil {
							f.Annotations = make(map[string]any)
						}
						f.Annotations[graphql.AnnotationName] = ann
					}
				}
			}
...
//
//	ex, err := graphqlgen.NewExtension(
//	    graphqlgen.WithSchemaHook(func(g *gen.Graph, schema *ast.Schema) error {
...
//
//	ex, err := graphqlgen.NewExtension(
//	    graphqlgen.WithOutputWriter(func(schema *ast.Schema) error {
...
//
//	ex, err := graphqlgen.NewExtension(
//	    graphqlgen.WithMapScalarFunc(func(t *gen.Type, f *gen.Field) string {
