	...
	typ, err := entgen.NewType(&entgen.Config{Package: "example.com/app/velox"}, &load.Schema{
		...
		Annotations: map[string]any{
			"graphql": graphql.Annotation{
				ResolverMappings: []graphql.ResolverMapping{graphql.Map("summary", "String").WithComment(hostile)},
				QueryField:       true,
				QueryFieldConfig: &graphql.QueryFieldSettings{Description: hostile},
			},
		},
	})
	if err != nil {
