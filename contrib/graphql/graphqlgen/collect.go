...
func collectionNodes(nodes []*gen.Type) []*gen.Type {
	return (&Generator{}).filterNodes(nodes, graphql.SkipType)
}
...
func markCollectFields(nodes []*gen.Type) {
	for _, t := range collectionNodes(nodes) {
		m, ok := t.Annotations[graphql.AnnotationName].(map[string]any)
		if !ok {
...
func (g *Generator) genEntityCollectionInit(f *jen.File, t *gen.Type, metaVar string) {
	...
	filteredFields := g.filterFields(t.Fields, graphql.SkipType)

	// Collect edges.
	filteredEdges := g.filterEdges(t.Edges)
