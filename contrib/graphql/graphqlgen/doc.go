// Package graphqlgen generates GraphQL for velox schemas: the SDL, and the
// Go the schema binds to (filters, pagination, mutation inputs, field
// collection). It is a velox compiler extension, run from generate.go; the
// annotations schemas use are package graphql, which is kept free of this
// generator because every server links what a schema imports.
//
...
//   - (for gqlgen servers, gqlgentx runs mutations in a transaction)
//
// # Usage
//
