...
func TestSubscriptionAnnotation_WithDescription(t *testing.T) {
	ann := graphql.Subscription(
		graphql.SubscriptionField("onPostPublished", "Post!").WithDescription("Fires when a post is published"),
	)
...
func TestSubscriptionAnnotation_WithArgs(t *testing.T) {
	ann := graphql.Subscription(
		graphql.SubscriptionField("onUserUpdated", "User!").WithArgs("id: ID!"),
	)
...
func TestSubscriptionAnnotation_Multiple(t *testing.T) {
	ann := graphql.Subscription(
		graphql.SubscriptionField("onUserCreated", "User!"),
		graphql.SubscriptionField("onUserDeleted", "User!"),
	)
	require.Len(t, ann.Subscriptions, 2)
}

func TestSubscriptionAnnotation_MergeDeduplicates(t *testing.T) {
	a := graphql.Subscription(graphql.SubscriptionField("onUserCreated", "User!"))
	b := graphql.Subscription(graphql.SubscriptionField("onUserDeleted", "User!"))
	merged := a.Merge(b).(graphql.Annotation)
		...
		Nodes: []*entgen.Type{
			{
				...
				Annotations: map[string]any{
					graphql.AnnotationName: graphql.Annotation{
						Subscriptions: []graphql.SubscriptionConfig{
