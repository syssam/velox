...
func (g *Generator) genEdgeField(_ *gen.Type, e *gen.Edge) string {
	...
	if e.Unique {
		...
		// Plan 3 (2026-04-25): the entity method (*Parent).Edge(ctx, after,
		// first, before, last, orderBy, where *filter.XxxWhereInput) carries
		// the `where` arg directly, so gqlgen autobinds without needing a
		// @goField(forceResolver: true) workaround.
	} else {
		fieldDef = fmt.Sprintf("%s: [%s!]!", name, targetType)
	}
...
func (g *Generator) genCreateInput(t *gen.Type) string {
	...
	fields := g.filterFields(t.Fields, graphql.SkipMutationCreateInput)
...
func (g *Generator) genUpdateInput(t *gen.Type) string {
	...
	fields := g.filterFields(t.Fields, graphql.SkipMutationUpdateInput)
	for _, f := range fields {
...
`, inputName, typeName)
	...
	// Add @goModel directive for autobind — WhereInput is defined in filter sub-package.
	if g.config.ORMPackage != "" {
		fmt.Fprintf(&buf, "input %s @goModel(model: \"%s/filter.%s\") {\n", inputName, g.config.ORMPackage, inputName)
	} else {
		fmt.Fprintf(&buf, "input %s {\n", inputName)
	}
...
`, typeName)
	...
	fields := g.filterFields(t.Fields, graphql.SkipOrderField)
	for _, f := range fields {
