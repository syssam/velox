...
func TestGenerator_GraphqlFieldName(t *testing.T) {
	...
	t.Run("CustomFieldName", func(t *testing.T) {
		f := &entgen.Field{
			Name: "user_name",
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{FieldName: "name"},
			},
		}
...
func TestGenerator_GraphqlFieldType_CustomAnnotation(t *testing.T) {
	...
	f := &entgen.Field{
		...
		Annotations: map[string]any{
			graphql.AnnotationName: &graphql.Annotation{Type: "ID"},
		},
	}
...
func TestGenerator_GraphqlInputFieldType(t *testing.T) {
	...
	t.Run("ValidInputCustomType", func(t *testing.T) {
		f := &entgen.Field{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{Type: "JSON"},
			},
		}
	...
	t.Run("ObjectTypeCustomAnnotation_FallsBackToJSON", func(t *testing.T) {
		f := &entgen.Field{
			...
			Type: &field.TypeInfo{Type: field.TypeJSON},
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{Type: "Address"},
			},
		}
...
func TestExtractGraphQLAnnotation(t *testing.T) {
	t.Run("NilMap", func(t *testing.T) {
		...
		assert.Equal(t, graphql.Annotation{}, ann)
	})
	...
	t.Run("PointerValue", func(t *testing.T) {
		ann := extractGraphQLAnnotation(map[string]any{
			graphql.AnnotationName: &graphql.Annotation{RelayConnection: true},
		})
	...
	t.Run("NilPointer", func(t *testing.T) {
		ann := extractGraphQLAnnotation(map[string]any{
			graphql.AnnotationName: (*graphql.Annotation)(nil),
		})
		assert.Equal(t, graphql.Annotation{}, ann)
	})
}
...
func TestGenerator_GenEnumType_CustomEnumValues(t *testing.T) {
	...
	f := &entgen.Field{
		...
		Annotations: map[string]any{
			graphql.AnnotationName: &graphql.Annotation{
...
func TestGenerator_ValidateResolverMappings(t *testing.T) {
	...
	t.Run("ValidMappings", func(t *testing.T) {
		typ := &entgen.Type{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{
					ResolverMappings: []graphql.ResolverMapping{
...
func TestGenerator_GetOrderFieldName(t *testing.T) {
	...
	t.Run("CustomAnnotation", func(t *testing.T) {
		f := &entgen.Field{
			Name: "email",
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{OrderField: "EMAIL_ADDRESS"},
			},
		}
...
func TestGenerator_GenOrderBy_MultiOrder(t *testing.T) {
	userType := &entgen.Type{
		...
		Annotations: map[string]any{
			graphql.AnnotationName: &graphql.Annotation{MultiOrder: true},
		},
	}
...
func TestGenerator_FieldInCreateInput(t *testing.T) {
	...
	t.Run("ExplicitAnnotation_Overrides", func(t *testing.T) {
		f := &entgen.Field{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{
					FieldMutationOps:    graphql.IncludeCreate,
...
func TestGenerator_TypeDirectives(t *testing.T) {
	...
	t.Run("SimpleDirective", func(t *testing.T) {
		typ := &entgen.Type{
			...
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{
					Directives: []graphql.Directive{{Name: "deprecated"}},
				},
			},
		}
...
func TestGenerator_HasRelayConnection(t *testing.T) {
	...
	t.Run("QueryFieldOverridesRelayDefault", func(t *testing.T) {
		typ := &entgen.Type{
			Name: "User",
			Annotations: map[string]any{
				graphql.AnnotationName: &graphql.Annotation{QueryField: true}, // QueryField without RelayConnection
			},
		}
		g := &entgen.Graph{
			Config: &entgen.Config{Package: "example/ent"},
...
//
// Regression class blocked: someone re-adds WithXxxPredicate / a
// `Predicate` field on PagerConfig as a "typed convenience" — that brings
// back the silent-double-filter footgun (both options applied additively
// when set) and a duplicated public API surface for one semantic.
func TestPagination_OnlyFilterOption_NoPredicate(t *testing.T) {
	typ := &entgen.Type{
		Name: "User",
		ID:   &entgen.Field{Name: "id", Type: &field.TypeInfo{Type: field.TypeInt64}},
		Fields: []*entgen.Field{
	...
	// WithUserFilter must still be there (the surviving option).
	assert.Contains(t, typesCode, "func WithUserFilter(filter func() (predicate.User, error)) UserPaginateOption",
		"WithUserFilter is the sole public predicate-threading option")

	// Negative invariants: no Predicate field, no WithXxxPredicate function.
	assert.NotContains(t, typesCode, "Predicate predicate.User",
		"PagerConfig must NOT carry a Predicate field — collapsed into Filter")
	assert.NotContains(t, typesCode, "func WithUserPredicate",
		"WithUserPredicate must not be emitted — collapsed into WithUserFilter")

	// Paginate body — must NOT have the cfg.Predicate apply block.
	pagFile := gen.genEntityPagination(typ)
	var pagBuf bytes.Buffer
	if err := pagFile.Render(&pagBuf); err != nil {
		t.Fatalf("render paginate: %v", err)
	...
	assert.NotContains(t, pagCode, "cfg.Predicate",
		"Paginate body must not reference cfg.Predicate — the field is gone")
	assert.NotContains(t, pagCode, "q.Where(cfg.Predicate)",
		"Paginate body must not apply cfg.Predicate — collapsed into Filter path")
}
...
func TestGenerator_ValidateTagEmission(t *testing.T) {
	userType := &entgen.Type{
		...
		Annotations: map[string]any{
			graphql.AnnotationName: graphql.Mutations(graphql.MutationCreate(), graphql.MutationUpdate()),
		},
	}
...
func TestGenerator_GenEdgeField_SkipOrderField(t *testing.T) {
	postType := &entgen.Type{
		...
		ID:   &entgen.Field{Name: "id", Type: &field.TypeInfo{Type: field.TypeInt64}},
		Annotations: map[string]any{
			graphql.AnnotationName: &graphql.Annotation{Skip: graphql.SkipOrderField},
		},
	}
	userType := &entgen.Type{
		Name: "User",
