...
// for a single entity. After Phase B (cycle-break), the file is emitted into
// client/{entity}/ (package {entity}client) — the same package as the
// mutation/create/update builder types it references, so those refs stay
// local.
//
// Enum types referenced in input structs still live in the {entity}/ leaf
// and get qualified via the regular generator paths (goInputFieldType uses
// g.entityPkgPath(t) which resolves to the leaf).
//
// SetInput methods are generated here because both input and builder types
// live in client/{entity}/.
...
func (g *Generator) genEntityMutationInputImpl(t *gen.Type) *jen.File {
	clientPkgPath := g.clientPkgPath(t)
	f := jen.NewFilePathName(clientPkgPath, g.clientPkgName(t))
...
func (g *Generator) edgeSkippedInInput(edge *gen.Edge, isCreate bool) bool {
	...
	if isCreate {
		return skip.Is(graphql.SkipMutationCreateInput)
	}
	return skip.Is(graphql.SkipMutationUpdateInput)
}
