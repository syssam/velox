...
func TestGenCreateInput_Omittable(t *testing.T) {
	...
	memoField := &entgen.Field{
		...
		Annotations: map[string]any{
			"graphql": graphql.Annotation{Omittable: true},
		},
	}
	typ := &entgen.Type{
		Name:        "Invoice",
	...
	typ := &entgen.Type{
		...
		Fields: []*entgen.Field{
			...
			{Name: "memo", Type: &field.TypeInfo{Type: field.TypeString}, Optional: true, Nillable: true,
				Annotations: map[string]any{"graphql": graphql.Annotation{Omittable: true}}},
		},
		Annotations: ann,
	}
	g := newTestGenerator(typ)
		...
		Edges: []*entgen.Edge{
			{
				...
				Rel:    entgen.Relation{Type: entgen.M2M},
				Annotations: map[string]any{
					"graphql": graphql.Annotation{Skip: graphql.SkipMutationCreateInput},
				},
			},
			{
				...
				Rel:    entgen.Relation{Type: entgen.O2M},
				Annotations: map[string]any{
					"graphql": graphql.Annotation{Skip: graphql.SkipMutationUpdateInput},
				},
			},
			{
	...
	typ := &entgen.Type{
		...
		Annotations: map[string]any{
			"graphql": graphql.Mutations(
				graphql.MutationCreate().Description("Creates an invoice."),
