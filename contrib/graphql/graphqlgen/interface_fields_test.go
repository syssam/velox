...
func interfaceFixture() (*entgen.Graph, map[string]*entgen.Type) {
	...
	todo.Annotations = ann(graphql.Annotation{Implements: []string{"BookmarkItem"}})
	project.Annotations = ann(graphql.Annotation{Implements: []string{"BookmarkItem"}})
	...
	m2o := func(owner *entgen.Type, name string, target *entgen.Type, ifield string) *entgen.Edge {
		...
		if ifield != "" {
			inverse.Annotations = ann(graphql.Annotation{InterfaceField: ifield})
		}
...
func TestInterfaceFieldGroups_Errors(t *testing.T) {
	...
	g, types = interfaceGen(t)
	types["Todo"].Edges[0].Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{InterfaceField: "category"}}
...
func TestInterfaceField_RejectsReservedGoName(t *testing.T) {
	for _, name := range []string{"edges", "config", "unwrap", "string"} {
		g, types := interfaceGen(t)
		types["Todo"].Edges[0].Annotations = map[string]any{graphql.AnnotationName: graphql.Annotation{InterfaceField: name}}
		_, err := g.interfaceFieldGroups(types["Todo"])
