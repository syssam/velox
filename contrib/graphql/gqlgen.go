...
//
// Entity sub-packages are added via InjectEntityBindings.
func (c *GQLGenConfig) InjectVeloxBindings(ormPackage string, schemaPath string) {
	...
	c.SetModel(ScalarUUID, "github.com/99designs/gqlgen/graphql.UUID")
	...
	c.SetModel(ScalarJSON, "github.com/99designs/gqlgen/graphql.Map")
}

// InjectEntityBindings is a no-op. Entity sub-package types are bound via
// @goModel directives in the GraphQL schema, not autobind. Adding entity
// sub-packages to autobind causes collisions (e.g., tag.Type matches __Type).
func (c *GQLGenConfig) InjectEntityBindings(_ string, _ []string) {
	// No-op: @goModel directives handle all entity type binding.
}
