...
func TestBuilder_Err(t *testing.T) {
	...
	_, _ = b.Query()
	require.EqualError(t, b.Err(), "invalid\nunexpected\ninner")
	// Verify errors.Is works through the joined error.
	require.ErrorIs(t, b.Err(), b.errs[0])
}
...
func TestSelector_Clone_PreservesLock(t *testing.T) {
	s := Dialect(dialect.Postgres).
		Select("*").
		From(Table("users")).
		Where(EQ("id", 1)).
		ForUpdate(WithLockAction(NoWait), WithLockTables("users"))

	clone := s.Clone()

	query1, _ := s.Query()
	require.Contains(t, query1, "FOR UPDATE")
	require.Contains(t, query1, "NOWAIT")
	require.Contains(t, query1, `OF "users"`)

	query2, _ := clone.Query()
	require.Contains(t, query2, "FOR UPDATE")
	require.Contains(t, query2, "NOWAIT")
	require.Contains(t, query2, `OF "users"`)
}

func TestSelector_Clone_LockIsolation(t *testing.T) {
	s := Dialect(dialect.Postgres).
		Select("*").
		From(Table("users")).
		ForUpdate()
	...
	query1, _ := s.Query()
	require.Contains(t, query1, "FOR UPDATE")
	require.NotContains(t, query1, "FOR SHARE")

	query2, _ := clone.Query()
	require.Contains(t, query2, "FOR SHARE")
}

func TestSelector_WithContext_Nil(t *testing.T) {
	s := Select("*").From(Table("users"))
	// Should not panic on nil context.
	var nilCtx context.Context //nolint:SA1012 // intentionally testing nil context handling
	s = s.WithContext(nilCtx)
