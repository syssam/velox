...
func ValueIn(column string, args []any, opts ...Option) *sql.Predicate {
	if len(args) == 0 {
		return sql.P(func(b *sql.Builder) { b.WriteString("FALSE") })
	}
...
func ValueNotIn(column string, args []any, opts ...Option) *sql.Predicate {
	if len(args) == 0 {
		return sql.P(func(b *sql.Builder) { b.WriteString("TRUE") })
	}
...
// Query returns the SQL expression and arguments for this path.
