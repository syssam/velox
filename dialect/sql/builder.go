...
// user-inputs. ~All validations are expected to happen in the generated
// velox package.
//
// SECURITY: Never pass untrusted user input as table names, column names,
// or other identifiers. All user-supplied values MUST go through
// parameterized query arguments (Arg/Args methods), not string interpolation.
package sql
...
// have a unique constraint applied to trigger this behavior.
//
//	sql.Insert("users").
//		Columns("id", "name").
//		Values(1, "Mashraki").
...
func (u *UpdateSet) SetExcluded(name string) *UpdateSet {
	...
	case dialect.MySQL:
		u.UpdateBuilder.Set(name, ExprFunc(func(b *Builder) {
			b.WriteString("VALUES(").Ident(name).Byte(')')
		}))
	default:
		t := Dialect(u.dialect).Table("excluded")
...
func (u *UpdateBuilder) Add(column string, v any) *UpdateBuilder {
	...
	u.values = append(u.values, ExprFunc(func(b *Builder) {
		...
		b.Wrap(func(b *Builder) {
			b.Ident(Table(u.table).C(column)).Comma().Byte('0')
		})
		b.WriteString(" + ")
		b.Arg(v)
	}))
...
//
// SECURITY: The expression string is embedded as-is in the query.
// Always use ? placeholders and pass values via args to prevent
// SQL injection. Never interpolate untrusted input into exr.
//
//	sql.ExprP("A = ? AND B > ?", args...)  // SAFE: parameterized
...
// Query is not safe for concurrent use. Each call resets and rebuilds
// the internal builder state. Callers must not share a Predicate across goroutines.
...
func (p *Predicate) clone() *Predicate {
	...
	fns := make([]func(*Builder), len(p.fns))
	copy(fns, p.fns)
	return &Predicate{fns: fns}
}
...
// If ctx is nil, WithContext is a no-op and the selector retains
// its current context (or context.Background via [Selector.Context]).
...
func (s *Selector) Select(columns ...string) *Selector {
	s.selection = make([]selection, 0, len(columns))
	for _, c := range columns {
		if c != "" {
			s.selection = append(s.selection, selection{c: c})
		}
	}
	return s
}
...
// Returns empty string if no table is set or if the table type is not supported.
func (s *Selector) TableName() string {
	...
	case *queryView, *WithBuilder:
		return ""
	default:
		s.AddError(fmt.Errorf("unhandled TableView type %T", s.from[0]))
		return ""
	}
}
...
// Postgres and the SQL standard require parentheses in that case.
...
func (s *Selector) C(column string) string {
	...
	// No FROM table set; return the column identifier as-is.
	b := &Builder{dialect: s.dialect}
	b.Ident(column)
	return b.String()
}
...
func (s *Selector) Clone() *Selector {
	...
	var collected [][]*Predicate
	if len(s.collected) > 0 {
		collected = make([][]*Predicate, len(s.collected))
		for i, ps := range s.collected {
			collected[i] = append([]*Predicate{}, ps...)
		}
	}
	clone := &Selector{
		Builder:   s.clone(),
		...
		setOps:    append([]setOp{}, s.setOps...),
		prefix:    append(Queries{}, s.prefix...),
		collected: collected,
	}
	if s.lock != nil {
		lock := *s.lock
		lock.Tables = append([]string(nil), s.lock.Tables...)
		clone.lock = &lock
	}
	return clone
}
...
//
// SECURITY WARNING: Raw bypasses all quoting, escaping, and parameterization.
// Never pass untrusted input to this function. It is intended only for
// trusted SQL fragments generated by the framework itself (e.g., view
// definitions, expression defaults). Misuse with user-supplied input will
// result in SQL injection vulnerabilities.
...
// Use [Builder.Ident] for column names and [Builder.Arg] for values to ensure
// proper quoting and parameterization.
//
//	Update("users").
...
// Byte appends a byte to the builder and returns the builder for chaining.
...
// Err returns a joined error of all errors encountered during
...
func (b *Builder) WriteOp(op Op) *Builder {
	...
	default:
		b.AddError(fmt.Errorf("invalid op %d", op))
	}
	return b
}
...
func (b *Builder) SetDialect(d string) {
	b.dialect = d
}

// SetDialect sets the dialect for the Selector and propagates it to all FROM tables.
// This ensures that column references (via C()) use the correct quoting style
// even before Query() is called.
func (s *Selector) SetDialect(d string) {
...
// Note: errs are intentionally not copied. The clone is used for string
// building in Query() methods, and errors from the clone are propagated
// back to the original via AddError(b.Err()).
func (b Builder) clone() Builder {
	c := Builder{dialect: b.dialect, total: b.total, sb: &strings.Builder{}}
	if len(b.args) > 0 {
		c.args = make([]any, 0, len(b.args))
...
// isIdent reports if the given string is a dialect identifier by checking
// if it contains the appropriate quote character for the dialect.
