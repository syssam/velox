...
func (d *SQLite) init(ctx context.Context) error {
	...
	if !on {
		// foreign_keys pragma is off, either execute "PRAGMA foreign_keys=ON"
		// or add the following parameter in the connection string "_pragma=foreign_keys(1)".
		return fmt.Errorf("sqlite: foreign_keys pragma is off: missing %q in the connection string", "_pragma=foreign_keys(1)")
	}
	return nil
}
...
func (d *SQLite) atTypeC(c1 *Column, c2 *schema.Column) error {
	...
	default:
		pt, err := sqlite.ParseType(strings.ToLower(c1.typ))
		if err != nil {
			return err
		}
		c2.Type.Type = pt
...
func (*SQLite) atTypeRangeSQL(table string, ts ...string) string {
	for i := range ts {
		ts[i] = fmt.Sprintf("('%s')", ts[i])
	}
	return fmt.Sprintf("INSERT INTO `%s` (`type`) VALUES %s", table, strings.Join(ts, ", "))
}
...
func (tx *tx) QueryContext(ctx context.Context, query string, args ...any) (*stdsql.Rows, error) {
	...
	r, ok := rows.ColumnScanner.(*stdsql.Rows)
	if !ok {
