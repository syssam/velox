...
const (
	// TypeTable defines the default table name holding the type information
	// for global unique IDs (Relay Node interface).
	TypeTable = "velox_types"

	// EntTypeTable is the legacy table name used by Ent ORM. Use WithTypeTable(EntTypeTable)
	// when migrating from an existing Ent project to preserve the type allocations.
	EntTypeTable = "ent_types"
...
func NewTypesTable() *Table {
	return newTypesTable(TypeTable)
}

// newTypesTable returns a new types table with the given name.
func newTypesTable(name string) *Table {
...
// WithTypeTable sets the table name used for storing type allocations
// when global unique IDs are enabled. Defaults to "velox_types".
//
// Use WithTypeTable(EntTypeTable) when migrating from Ent to preserve
// existing type allocations.
func WithTypeTable(name string) MigrateOption {
	return func(a *Atlas) {
		a.typeTable = name
	}
}
