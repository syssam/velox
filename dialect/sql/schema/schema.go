...
import (
	...
	entdialect "github.com/syssam/velox/dialect"
...
type Table struct {
	...
	Pos         string // filename:line of the schema definition.
}
...
// SetAnnotation the sqlschema.Annotation on the table.
func (t *Table) SetAnnotation(ant *sqlschema.Annotation) *Table {
	t.Annotation = ant
	return t
}
...
// column is an alias for Column for internal use.
...
func (c *Column) ScanDefault(value string) error {
	switch {
	case strings.EqualFold(value, Null): // ignore.
	case c.IntType():
		v := &sql.NullInt64{}
...
// ReferenceOption is a type alias for sqlschema.CascadeAction to avoid
// duplicate definitions of the same cascade constants.
type ReferenceOption = sqlschema.CascadeAction

// Reference options — aliases for the canonical constants in sqlschema.
const (
	NoAction   ReferenceOption = sqlschema.NoAction
	Restrict   ReferenceOption = sqlschema.Restrict
	Cascade    ReferenceOption = sqlschema.Cascade
	SetNull    ReferenceOption = sqlschema.SetNull
	SetDefault ReferenceOption = sqlschema.SetDefault
)
...
type Index struct {
	...
	Annotation *sqlschema.IndexAnnotation // index annotation.
...
// DDLArgs holds arguments for DDL generation.
type DDLArgs struct {
