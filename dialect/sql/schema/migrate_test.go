...
func TestMigrate_DiffJoinTableAllocationBC(t *testing.T) {
	...
	for _, stmt := range []string{
		...
		"CREATE INDEX `long____________________________e6cadf8dda4152f7a881133374e3c902` ON `groups` (`id`);",
		...
		"CREATE TABLE `velox_types` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `type` text NOT NULL);",
		"CREATE UNIQUE INDEX `velox_types_type_key` ON `velox_types` (`type`);",
		"INSERT INTO `velox_types` (`type`) VALUES ('groups'), ('users'), ('user_groups');",
	...
	rows, err := db.QueryContext(context.Background(), "SELECT `type` from `velox_types` ORDER BY `id` ASC")
	require.NoError(t, err)
	var types []string
	for rows.Next() {
		var typ string
...
func TestMigrate_Diff(t *testing.T) {
	...
	changesSQL := strings.Join([]string{
		...
		"CREATE TABLE `velox_types` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `type` text NOT NULL);",
		"CREATE UNIQUE INDEX `velox_types_type_key` ON `velox_types` (`type`);",
		"INSERT INTO `velox_types` (`type`) VALUES ('groups'), ('users');",
		"",
	}, "\n")
	...
	require.NoError(t, err)
	require.NoError(t, m.Diff(ctx, tables...))
	requireFileEqual(t, filepath.Join(p, "changes.sql"), "INSERT INTO `velox_types` (`type`) VALUES ('groups'), ('users');\n")
	...
	requireFileEqual(t,
		filepath.Join(p, "changes_2.sql"), strings.Join([]string{
			...
			"INSERT INTO `velox_types` (`type`) VALUES ('pets');", "",
		}, "\n"))
