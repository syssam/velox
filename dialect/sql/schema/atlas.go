...
//
// Atlas is NOT safe for concurrent use. Callers must serialize access
// to Create, NamedDiff, VerifyTableRange, and other mutating methods.
type Atlas struct {
	mu         sync.Mutex     // serializes access to atDriver and sqlDialect
	atDriver   migrate.Driver // set/cleared within each operation
	sqlDialect sqlDialect     // set/cleared within each operation

	schema          string // schema to use
	indent          string // plan indentation
	typeTable       string // type table name for global unique ids
...
func (a *Atlas) NamedDiff(ctx context.Context, name string, tables ...*Table) error {
	...
	if a.universalID {
		tables = append(tables, newTypesTable(a.typeTable))
	}
	var (
		err  error
		plan *migrate.Plan
...
func (a *Atlas) create(ctx context.Context, tables ...*Table) (err error) {
	...
	var applier Applier = ApplyFunc(func(ctx context.Context, tx dialect.ExecQuerier, plan *migrate.Plan) error {
		for _, c := range plan.Changes {
			if execErr := tx.Exec(ctx, c.Cmd, c.Args, nil); execErr != nil {
				if c.Comment != "" {
					execErr = fmt.Errorf("%s: %w", c.Comment, execErr)
				}
				return execErr
			}
		}
...
func (a *Atlas) diff(ctx context.Context, name string, current, desired *schema.Schema, newTypes []string, opts ...migrate.PlanOption) (*migrate.Plan, error) {
	...
	if len(newTypes) > 0 {
		plan.Changes = append(plan.Changes, &migrate.Change{
			Cmd:     a.sqlDialect.atTypeRangeSQL(a.typeTable, newTypes...),
...
var errTypeTableNotFound = errors.New("type table not found")

// loadTypes loads the currently saved range allocations from the type table.
func (a *Atlas) loadTypes(ctx context.Context, conn dialect.ExecQuerier) ([]string, error) {
	// Fetch pre-existing type allocations.
	exists, err := a.sqlDialect.tableExist(ctx, conn, a.typeTable)
	if err != nil {
		return nil, err
	}
	if !exists {
	...
	query, args := entsql.Dialect(a.dialect).
		Select("type").From(entsql.Table(a.typeTable)).OrderBy(entsql.Asc("id")).Query()
...
func (d *db) QueryContext(ctx context.Context, query string, args ...any) (*sql.Rows, error) {
	...
	r, ok := rows.ColumnScanner.(*sql.Rows)
	if !ok {
		return nil, fmt.Errorf("unexpected ColumnScanner type %T", rows.ColumnScanner)
	}
	return r, nil
}
...
func (a *Atlas) realm(tables []*Table) (*schema.Realm, error) {
	...
	for _, et := range tables {
		...
		case a.universalID && et.Name != a.typeTable && len(et.PrimaryKey) == 1:
			r, err := a.pkRange(et)
			if err != nil {
				return nil, err
			}
...
func (a *Atlas) atDefault(c1 *Column, c2 *schema.Column) error {
	...
	default:
		...
		case field.TypeJSON:
			...
			c2.SetDefault(&schema.Literal{V: s})
		case field.TypeString, field.TypeEnum:
			// Use Literal for string types - Atlas handles quoting internally.
			v, ok := c1.Default.(string)
			if !ok {
				return fmt.Errorf("invalid default value for string column %q: %v", c1.Name, c1.Default)
			}
			c2.SetDefault(&schema.Literal{V: v})
		default:
			// For non-string types (bool, int, float, uuid, time), use RawExpr.
			c2.SetDefault(&schema.RawExpr{X: fmt.Sprint(c1.Default)})
		}
	}
	return nil
}
...
func (a *Atlas) symbol(name string) string {
	...
	h := fnv.New128()
	h.Write([]byte(name))
	return fmt.Sprintf("%s_%x", name[:size-33], h.Sum(nil))
}
