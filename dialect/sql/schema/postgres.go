...
func (d *Postgres) atTypeC(c1 *Column, c2 *schema.Column) error {
	...
	default:
		pt, err := postgres.ParseType(strings.ToLower(c1.typ))
		if err != nil {
			return err
		}
		c2.Type.Type = pt
...
func (*Postgres) atTypeRangeSQL(table string, ts ...string) string {
	for i := range ts {
		ts[i] = fmt.Sprintf("('%s')", ts[i])
	}
	//nolint:gocritic // Double quotes are intentional for PostgreSQL identifier quoting
