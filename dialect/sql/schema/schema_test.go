...
func TestCopyTables(t *testing.T) {
	...
	users.AddColumn(&Column{Name: "id", Type: field.TypeInt})
	users.AddColumn(&Column{Name: "name", Type: field.TypeString})
	users.AddColumn(&Column{Name: "spouse_id", Type: field.TypeInt})
	...
	users.SetAnnotation(&sqlschema.Annotation{Table: "Users"})
	pets := NewTable("pets")
	pets.AddColumn(&Column{Name: "id", Type: field.TypeInt})
	pets.AddColumn(&Column{Name: "name", Type: field.TypeString})
	pets.AddColumn(&Column{Name: "owner_id", Type: field.TypeInt})
...
func TestDDL(t *testing.T) {
	const (
		hash   = "f4cf9c0a6991157036b9d4bdc7c11aff"
		idx    = "super_duper_mega_ultra_hyper_giga_colossal_unbelievably_long_index_name"
		hashMY = "super_duper_mega_ultra_hyper_gi_f4cf9c0a6991157036b9d4bdc7c11aff"
		hashPG = "super_duper_mega_ultra_hyper_g_f4cf9c0a6991157036b9d4bdc7c11aff"
	)
	tbls := func() []*Table {
		users := &Table{
			Name: "users",
		...
		petsWithoutFur := &Table{
			...
			Annotation: sqlschema.View("SELECT id, name, owner_id FROM pets"),
		}
		petNames := &Table{
			Name:    "pet_names",
			Pos:     "pets.go:45",
			...
			Annotation: sqlschema.ViewFor(dialect.Postgres, func(s *sql.Selector) {
