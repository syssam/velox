...
func (d *MySQL) atTypeC(c1 *Column, c2 *schema.Column) error {
	...
	default:
		pt, err := mysql.ParseType(strings.ToLower(c1.typ))
		if err != nil {
			return err
		}
		c2.Type.Type = pt
...
func (d *MySQL) atImplicitIndexName(idx *Index, c1 *Column) bool {
	...
	i, err := strconv.ParseInt(strings.TrimPrefix(idx.Name, c1.Name+"_"), 10, 64)
	return err == nil && i > 1
}
...
func (*MySQL) atTypeRangeSQL(table string, ts ...string) string {
	for i := range ts {
