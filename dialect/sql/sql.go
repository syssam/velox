...
// Generic version for user code that passes typed slices.
func FieldIn[T any](name string, vs ...T) func(*Selector) {
	v := make([]any, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return FieldInValues(name, v...)
}
...
func FieldNotIn[T any](name string, vs ...T) func(*Selector) {
	...
	return FieldNotInValues(name, v...)
}

// FieldInValues is the non-generic version of FieldIn. Used by generated code
// to avoid generic monomorphization overhead.
func FieldInValues(name string, vs ...any) func(*Selector) {
	return func(s *Selector) {
		s.Where(In(s.C(name), vs...))
	}
}

// FieldNotInValues is the non-generic version of FieldNotIn. Used by generated code
// to avoid generic monomorphization overhead.
func FieldNotInValues(name string, vs ...any) func(*Selector) {
	return func(s *Selector) {
...
// combinePredicates is a helper that collects predicates and applies a combinator function.
func combinePredicates[P ~func(*Selector)](predicates []P, combine func([]*Predicate) *Predicate) func(*Selector) {
	return func(s *Selector) {
		s.CollectPredicates()
...
func AndPredicates[P ~func(*Selector)](predicates ...P) func(*Selector) {
	return combinePredicates(predicates, func(preds []*Predicate) *Predicate {
		if len(preds) == 1 {
			return preds[0]
		}
		return And(preds...)
...
func NotPredicates[P ~func(*Selector)](predicates ...P) func(*Selector) {
	return combinePredicates(predicates, func(preds []*Predicate) *Predicate {
		...
		return Not(And(preds...))
	})
}

// combineFuncs is the non-generic version of combinePredicates for func(*Selector) slices.
// Used by generated code to avoid generic monomorphization overhead.
func combineFuncs(predicates []func(*Selector), combine func([]*Predicate) *Predicate) func(*Selector) {
	return func(s *Selector) {
		s.CollectPredicates()
		for _, p := range predicates {
			p(s)
...
// AndFuncs is the non-generic version of AndPredicates. Used by generated code.
func AndFuncs(predicates ...func(*Selector)) func(*Selector) {
	return combineFuncs(predicates, func(preds []*Predicate) *Predicate {
		if len(preds) == 1 {
			return preds[0]
		}
		return And(preds...)
...
// OrFuncs is the non-generic version of OrPredicates. Used by generated code.
func OrFuncs(predicates ...func(*Selector)) func(*Selector) {
	return combineFuncs(predicates, func(preds []*Predicate) *Predicate {
		if len(preds) == 1 {
			return preds[0]
		}
...
// NotFuncs is the non-generic version of NotPredicates. Used by generated code.
func NotFuncs(predicates ...func(*Selector)) func(*Selector) {
	return combineFuncs(predicates, func(preds []*Predicate) *Predicate {
		if len(preds) == 1 {
			return Not(preds[0])
		}
...
// PredicateAnd returns a predicate that ANDs the given typed predicates.
// Accepts any type whose underlying type is func(*Selector), allowing
// generated per-entity predicate types to use this directly.
func PredicateAnd[P ~func(*Selector)](ps ...P) P {
	return func(s *Selector) {
		fns := make([]func(*Selector), len(ps))
		for i, p := range ps {
			fns[i] = (func(*Selector))(p)
		}
		AndFuncs(fns...)(s)
	}
}

// PredicateOr returns a predicate that ORs the given typed predicates.
// Accepts any type whose underlying type is func(*Selector), allowing
// generated per-entity predicate types to use this directly.
func PredicateOr[P ~func(*Selector)](ps ...P) P {
	return func(s *Selector) {
		fns := make([]func(*Selector), len(ps))
		for i, p := range ps {
			fns[i] = (func(*Selector))(p)
...
// PredicateNot returns a predicate that NOTs the given typed predicates.
// Accepts any type whose underlying type is func(*Selector), allowing
// generated per-entity predicate types to use this directly.
func PredicateNot[P ~func(*Selector)](ps ...P) P {
	return func(s *Selector) {
		fns := make([]func(*Selector), len(ps))
		for i, p := range ps {
			fns[i] = (func(*Selector))(p)
