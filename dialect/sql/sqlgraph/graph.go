...
// of sql-based databases for velox codegen.
package sqlgraph
...
func Neighbors(d string, s *Step) (q *sql.Selector) {
...
func (u *updater) nodes(ctx context.Context, drv dialect.Driver) (int, error) {
	...
	affected, err := func() (int, error) {
		...
		defer rows.Close()
		if qerr = sql.ScanSlice(rows, &ids); qerr != nil {
			return 0, fmt.Errorf("scan node ids: %w", qerr)
		}
		...
		if _, qerr = u.updateTable(ctx, update); qerr != nil {
			return 0, qerr
		}
		if qerr = u.setExternalEdges(ctx, ids, addEdges, clearEdges); qerr != nil {
			return 0, qerr
		}
		return len(ids), nil
	}()
...
func (u *updater) setExternalEdges(ctx context.Context, ids []driver.Value, addEdges, clearEdges map[Rel][]*EdgeSpec) error {
	...
	return u.addFKEdges(ctx, ids, append(addEdges[O2M], addEdges[O2O]...))
}
...
// scanInsertedID translates the result of scanning a RETURNING clause
// into the final error returned to the caller. ON CONFLICT DO NOTHING
// legitimately returns zero rows when a duplicate is hit, which the
// scan layer reports as sql.ErrNoRows; for any insert that carries an
// ON CONFLICT clause we treat that as success and leave c.ID.Value
// unset (callers cannot recover the existing row's ID via RETURNING
// in this case — they would need a follow-up SELECT). All other errors
// propagate unchanged.
func (c *creator) scanInsertedID(err error) error {
	if err == nil {
		return nil
	}
...
func (c *batchCreator) mayTx(ctx context.Context, drv dialect.Driver) (dialect.Tx, error) {
	needsTx := slices.ContainsFunc(c.Nodes, func(node *CreateSpec) bool {
		return slices.ContainsFunc(node.Edges, isExternalEdge)
	})
	if needsTx {
		return drv.Tx(ctx)
	}
