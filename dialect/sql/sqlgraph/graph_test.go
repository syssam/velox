...
func TestQueryNodes(t *testing.T) {
	...
	mock.ExpectQuery(escape("SELECT DISTINCT `users`.`id`, `users`.`age`, `users`.`name`, `users`.`fk1`, `users`.`fk2` FROM `users` WHERE `age` < ? ORDER BY `id` LIMIT 3 OFFSET 4 FOR UPDATE NOWAIT")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "age", "name", "fk1", "fk2"}).
			...
			AddRow(2, 20, "", 0, 0).
			AddRow(3, 30, "a8m", 1, 1))
	mock.ExpectQuery(escape("SELECT COUNT(*) FROM (SELECT DISTINCT `users`.`id` FROM `users` WHERE `age` < ? LIMIT 3 OFFSET 4 FOR UPDATE NOWAIT) AS `t1`")).
		WithArgs(40).
...
func TestIsConstraintError(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "MySQL Unique",
			errMessage:         `insert node to table "file_types": UNIQUE constraint failed: file_types.name velox: constraint failed: insert node to table "file_types": UNIQUE constraint failed: file_types.name`,
			expectedConstraint: true,
			expectedFK:         false,
			expectedUnique:     true,
			expectedCheck:      false,
		...
		},
		{
			...
			errMessage:         `insert node to table "file_types": pq: duplicate key value violates unique constraint "file_types_name_key" velox: constraint failed: insert node to table "file_types": pq: duplicate key value violates unique constraint "file_types_name_key"`,
			expectedConstraint: true,
			expectedFK:         false,
			expectedUnique:     true,
			expectedCheck:      false,
...
// mockTxDriver simulates a driver that is already inside a transaction.
// This is used to test that BatchCreate, CreateNode, UpdateNode, etc.
// don't commit/rollback the outer transaction.
// mockTxDriver simulates the generated txDriver behavior where Tx() returns itself
// and Commit/Rollback are no-ops. This mimics being inside a transaction where internal
// builders should not affect the outer transaction's lifecycle.
type mockTxDriver struct {
	dialect.Driver
}

// Tx returns itself to simulate txDriver behavior (returning the wrapper, not a new transaction).
func (m *mockTxDriver) Tx(_ context.Context) (dialect.Tx, error) {
	return m, nil
}

// Commit is a no-op to simulate txDriver behavior.
func (m *mockTxDriver) Commit() error {
	return nil
}

// Rollback is a no-op to simulate txDriver behavior.
func (m *mockTxDriver) Rollback() error {
	return nil
}

// TestBatchCreateInsideTransaction verifies that BatchCreate does not commit
// the outer transaction when the driver is already a transaction.
func TestBatchCreateInsideTransaction(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)

	// Set up expectations - no Begin/Commit should be called since we're already in a tx
	mock.ExpectExec(escape("INSERT INTO `users` (`name`) VALUES (?), (?)")).
		WithArgs("a8m", "nati").
		WillReturnResult(sqlmock.NewResult(1, 2))

	// Create a mock transaction driver (simulating being inside a transaction)
	baseDriver := sql.OpenDB(dialect.MySQL, db)
	txDriver := &mockTxDriver{Driver: baseDriver}
	...
	err = BatchCreate(context.Background(), txDriver, spec)
	require.NoError(t, err)
...
// TestCreateNodeInsideTransaction verifies that CreateNode does not commit
// the outer transaction when the driver is already a transaction.
func TestCreateNodeInsideTransaction(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)

	// Expect insert and external edge update (which normally would require a transaction)
	mock.ExpectExec(escape("INSERT INTO `users` (`name`) VALUES (?)")).
		WithArgs("a8m").
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(escape("UPDATE `pets` SET `owner_id` = ? WHERE `id` = ? AND `owner_id` IS NULL")).
	...
	// Create a mock transaction driver
	baseDriver := sql.OpenDB(dialect.MySQL, db)
	txDriver := &mockTxDriver{Driver: baseDriver}
	...
	err = CreateNode(context.Background(), txDriver, spec)
	require.NoError(t, err)
...
// TestUpdateNodeInsideTransaction verifies that UpdateNode does not commit
// the outer transaction when the driver is already a transaction.
func TestUpdateNodeInsideTransaction(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)

	mock.ExpectExec(escape("UPDATE `users` SET `name` = ? WHERE `id` = ?")).
		WithArgs("a8m", 1).
		WillReturnResult(sqlmock.NewResult(1, 1))
	...
	err = UpdateNode(context.Background(), txDriver, spec)
	require.NoError(t, err)
