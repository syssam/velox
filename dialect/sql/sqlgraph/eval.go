...
func (g *Schema) EvalP(nodeType string, p querylanguage.P, selector *sql.Selector) error {
	var node *Node
	for i := range g.Nodes {
...
const FuncSelector querylanguage.Func = "func_selector"
...
func WrapFunc(s func(*sql.Selector)) *querylanguage.CallExpr {
	return &querylanguage.CallExpr{
		Func: FuncSelector,
		Args: []querylanguage.Expr{wrappedFunc{Func: s}},
	}
}

var (
	binary = [...]sql.Op{
		querylanguage.OpEQ:    sql.OpEQ,
		querylanguage.OpNEQ:   sql.OpNEQ,
		querylanguage.OpGT:    sql.OpGT,
		querylanguage.OpGTE:   sql.OpGTE,
		querylanguage.OpLT:    sql.OpLT,
		querylanguage.OpLTE:   sql.OpLTE,
		querylanguage.OpIn:    sql.OpIn,
		querylanguage.OpNotIn: sql.OpNotIn,
	}
	nary = [...]func(...*sql.Predicate) *sql.Predicate{
		querylanguage.OpAnd: sql.And,
		querylanguage.OpOr:  sql.Or,
	}
	strFunc = map[querylanguage.Func]func(string, string) *sql.Predicate{
		querylanguage.FuncContains:     sql.Contains,
		querylanguage.FuncContainsFold: sql.ContainsFold,
		querylanguage.FuncEqualFold:    sql.EqualFold,
		querylanguage.FuncHasPrefix:    sql.HasPrefix,
		querylanguage.FuncHasSuffix:    sql.HasSuffix,
	}
	nullFunc = [...]func(string) *sql.Predicate{
		querylanguage.OpEQ:  sql.IsNull,
		querylanguage.OpNEQ: sql.NotNull,
	}
)
...
func evalExpr(context *Node, selector *sql.Selector, expr querylanguage.Expr) (p *sql.Predicate, err error) {
	ex := &state{
		context:  context,
		selector: selector,
	}
...
func (e *state) evalExpr(expr querylanguage.Expr) *sql.Predicate {
	...
	case *querylanguage.CallExpr:
		switch expr.Func {
		case querylanguage.FuncHasPrefix, querylanguage.FuncHasSuffix, querylanguage.FuncContains, querylanguage.FuncEqualFold, querylanguage.FuncContainsFold:
			expect(len(expr.Args) == 2, "invalid number of arguments for %s", expr.Func)
			f, ok := expr.Args[0].(*querylanguage.Field)
			expect(ok, "*querylanguage.Field, got %T", expr.Args[0])
			v, ok := expr.Args[1].(*querylanguage.Value)
			expect(ok, "*querylanguage.Value, got %T", expr.Args[1])
			s, ok := v.V.(string)
			expect(ok, "string value, got %T", v.V)
			return strFunc[expr.Func](e.field(f), s)
		case querylanguage.FuncHasEdge:
			expect(len(expr.Args) > 0, "invalid number of arguments for %s", expr.Func)
			edge, ok := expr.Args[0].(*querylanguage.Edge)
			expect(ok, "*querylanguage.Edge, got %T", expr.Args[0])
			return e.evalEdge(edge.Name, expr.Args[1:]...)
		}
	}
	panic(evalError{fmt.Sprintf("unsupported expression type %T", expr)})
}

// evalBinary evaluates binary expressions.
func (e *state) evalBinary(expr *querylanguage.BinaryExpr) *sql.Predicate {
	switch expr.Op {
	case querylanguage.OpOr:
		return sql.Or(e.evalExpr(expr.X), e.evalExpr(expr.Y))
	case querylanguage.OpAnd:
		return sql.And(e.evalExpr(expr.X), e.evalExpr(expr.Y))
	case querylanguage.OpEQ, querylanguage.OpNEQ:
		if expr.Y == (*querylanguage.Value)(nil) {
			f, ok := expr.X.(*querylanguage.Field)
			expect(ok, "*querylanguage.Field, got %T", expr.Y)
			return nullFunc[expr.Op](e.field(f))
		}
		fallthrough
	default:
		field, ok := expr.X.(*querylanguage.Field)
		expect(ok, "expr.X to be *querylanguage.Field (got %T)", expr.X)
		_, ok = expr.Y.(*querylanguage.Field)
		if !ok {
			_, ok = expr.Y.(*querylanguage.Value)
		}
		expect(ok, "expr.Y to be *querylanguage.Field or *querylanguage.Value (got %T)", expr.Y)
		...
		default:
			panic(evalError{fmt.Sprintf("unreachable: unexpected expr.Y type %T", expr.Y)})
		}
	}
}

// evalEdge evaluates has-edge and has-edge-with calls.
func (e *state) evalEdge(name string, exprs ...querylanguage.Expr) *sql.Predicate {
	...
	HasNeighborsWith(selector, step, func(s *sql.Selector) {
		for _, expr := range exprs {
			if cx, ok := expr.(*querylanguage.CallExpr); ok && cx.Func == FuncSelector {
...
func (e *state) field(f *querylanguage.Field) string {
...
//nolint:gocritic // ptrToRefParam: pointer-to-error is intentional for recover pattern
