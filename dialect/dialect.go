...
type DebugDriver struct {
	...
	log    func(context.Context, ...any) // log function. defaults to slog.Info.
}
...
func Debug(d Driver, logger ...func(...any)) Driver {
	logf := func(v ...any) { slog.Info(fmt.Sprint(v...)) }
	if len(logger) == 1 {
		logf = logger[0]
	}
...
func (d *DebugDriver) Tx(ctx context.Context) (Tx, error) {
	...
	return &DebugTx{Tx: tx, id: id, log: d.log}, nil
}
...
// Uses context.Background() because Commit/Rollback have no context parameter
// in the dialect.Tx interface, and storing the original transaction context
// would violate Go's guideline against keeping contexts in structs.
func (d *DebugTx) Commit() error {
	d.log(context.Background(), fmt.Sprintf("Tx(%s): committed", d.id))
	return d.Tx.Commit()
}

// Rollback logs this step and calls the underlying transaction Rollback method.
// See Commit for context handling rationale.
func (d *DebugTx) Rollback() error {
	d.log(context.Background(), fmt.Sprintf("Tx(%s): rolled back", d.id))
