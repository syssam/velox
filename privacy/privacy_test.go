...
// queryRuleFunc wraps privacy.QueryRuleFunc for use in tests.
// This is a local alias to avoid updating all test call sites.
type queryRuleFunc = privacy.QueryRuleFunc
...
func TestPoliciesContextPropagation(t *testing.T) {
	...
	err := policy.EvalQuery(ctx, &mockQuery{})
	// Allow is translated to nil by QueryPolicy.EvalQuery (correct Ent semantics)
	assert.NoError(t, err)
}
