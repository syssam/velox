package schema

import (
	"github.com/syssam/velox/contrib/graphql"
	"github.com/syssam/velox"
	"github.com/syssam/velox/schema"
	"github.com/syssam/velox/schema/edge"
	"github.com/syssam/velox/schema/field"
	"github.com/syssam/velox/schema/mixin"
)

type ShipmentItem struct{ velox.Schema }

func (ShipmentItem) Mixin() []velox.Mixin { return []velox.Mixin{mixin.Time{}} }

func (ShipmentItem) Fields() []velox.Field {
	return []velox.Field{
		field.Text("description").Optional().Nillable(),
		field.Enum("priority").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("PRIORITY")),
		field.Bool("active").Default(false),
		field.Enum("status").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("STATUS")),
		field.String("title").NotEmpty().MaxLen(200).Annotations(graphql.OrderField("TITLE")),
		field.Int("sort_order").Default(0).Annotations(graphql.OrderField("SORT_ORDER")),
		field.String("email").MaxLen(255).Annotations(graphql.OrderField("EMAIL")),
	}
}

func (ShipmentItem) Edges() []velox.Edge {
	return []velox.Edge{
		edge.To("comments", Comment.Type).Annotations(graphql.RelayConnection()),
		edge.To("plans", Plan.Type).Annotations(graphql.RelayConnection()),
		edge.To("milestone_links", Milestone.Type).Annotations(graphql.RelayConnection()),
	}
}

func (ShipmentItem) Annotations() []schema.Annotation {
	return []schema.Annotation{
		graphql.RelayConnection(),
		graphql.QueryField(),
		graphql.Mutations(graphql.MutationCreate(), graphql.MutationUpdate()),
	}
}
