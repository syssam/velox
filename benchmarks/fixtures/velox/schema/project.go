package schema

import (
	"github.com/syssam/velox/contrib/graphql"
	"github.com/syssam/velox"
	"github.com/syssam/velox/schema"
	"github.com/syssam/velox/schema/edge"
	"github.com/syssam/velox/schema/field"
	"github.com/syssam/velox/schema/mixin"
)

type Project struct{ velox.Schema }

func (Project) Mixin() []velox.Mixin { return []velox.Mixin{mixin.Time{}} }

func (Project) Fields() []velox.Field {
	return []velox.Field{
		field.Text("notes").Optional().Nillable(),
		field.Float("amount").Default(0).Annotations(graphql.OrderField("AMOUNT")),
		field.Enum("status").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("STATUS")),
		field.Enum("priority").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("PRIORITY")),
		field.Int("quantity").Default(0).Annotations(graphql.OrderField("QUANTITY")),
		field.Text("description").Optional().Nillable(),
		field.JSON("metadata", map[string]any{}).Optional(),
		field.Bool("active").Default(false),
	}
}

func (Project) Edges() []velox.Edge {
	return []velox.Edge{
		edge.To("invoices", Invoice.Type).Annotations(graphql.RelayConnection()),
		edge.To("apikeys", ApiKey.Type).Annotations(graphql.RelayConnection()),
		edge.To("shipment_links", Shipment.Type).Annotations(graphql.RelayConnection()),
	}
}

func (Project) Annotations() []schema.Annotation {
	return []schema.Annotation{
		graphql.RelayConnection(),
		graphql.QueryField(),
		graphql.Mutations(graphql.MutationCreate(), graphql.MutationUpdate()),
	}
}
