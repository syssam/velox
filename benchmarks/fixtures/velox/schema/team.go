package schema

import (
	"github.com/syssam/velox"
	"github.com/syssam/velox/contrib/graphql"
	"github.com/syssam/velox/schema"
	"github.com/syssam/velox/schema/edge"
	"github.com/syssam/velox/schema/field"
	"github.com/syssam/velox/schema/mixin"
)

type Team struct{ velox.Schema }

func (Team) Mixin() []velox.Mixin { return []velox.Mixin{mixin.Time{}} }

func (Team) Fields() []velox.Field {
	return []velox.Field{
		field.String("url").MaxLen(500).Optional().Nillable().Annotations(graphql.OrderField("URL")),
		field.String("code").MaxLen(50).Annotations(graphql.OrderField("CODE")),
		field.Float("score").Default(0).Optional().Nillable().Annotations(graphql.OrderField("SCORE")),
		field.String("email").MaxLen(255).Annotations(graphql.OrderField("EMAIL")),
		field.Float("amount").Default(0).Annotations(graphql.OrderField("AMOUNT")),
		field.Enum("priority").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("PRIORITY")),
		field.Enum("status").Values("low", "medium", "high").Default("medium").Annotations(graphql.OrderField("STATUS")),
	}
}

func (Team) Edges() []velox.Edge {
	return []velox.Edge{
		edge.To("files", File.Type).Annotations(graphql.RelayConnection()),
		edge.To("customers", Customer.Type).Annotations(graphql.RelayConnection()),
		edge.To("attachment_links", Attachment.Type).Annotations(graphql.RelayConnection()),
	}
}

func (Team) Annotations() []schema.Annotation {
	return []schema.Annotation{
		graphql.RelayConnection(),
		graphql.QueryField(),
		graphql.Mutations(graphql.MutationCreate(), graphql.MutationUpdate()),
	}
}
