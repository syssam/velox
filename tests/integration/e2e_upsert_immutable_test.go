...
func TestUpsertUpdateNewValues_KeepsImmutableColumns(t *testing.T) {
	forEachDialect(t, func(t *testing.T, c *integration.Client) {
		...
		t.Run("returned_id_is_the_stored_row", func(t *testing.T) {
			...
			require.NoError(t, err)
			// MySQL has no RETURNING and reports only numeric auto-increment
			// IDs, so after a conflict on a UUID key the ID is unknown: the
			// zero UUID, never the generated key of a row that was not
			// inserted.
