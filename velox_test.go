...
func TestView(t *testing.T) {
	...
	// Test that View's view method is callable (Viewer interface)
	// The view() method is unexported, so we test via interface
	var viewer velox.Viewer = TestView{}
...
func TestQueryContext(t *testing.T) {
	...
	t.Run("Clone with all optional fields", func(t *testing.T) {
		t.Parallel()
		...
		cloned := original.Clone()
		require.NotNil(t, cloned)
		assert.Equal(t, "All", cloned.Op)
		assert.Equal(t, "User", cloned.Type)
		assert.Equal(t, original.Fields, cloned.Fields)
		require.NotNil(t, cloned.Unique)
		assert.Equal(t, true, *cloned.Unique)
		require.NotNil(t, cloned.Limit)
		assert.Equal(t, 50, *cloned.Limit)
		require.NotNil(t, cloned.Offset)
		assert.Equal(t, 100, *cloned.Offset)

		// Modifying cloned pointers should not affect original
		*cloned.Unique = false
		assert.True(t, *original.Unique)
		*cloned.Limit = 999
		assert.Equal(t, 50, *original.Limit)
		*cloned.Offset = 999
		assert.Equal(t, 100, *original.Offset)
	})
	...
	t.Run("Clone with nil optional fields", func(t *testing.T) {
		...
		cloned := original.Clone()
		require.NotNil(t, cloned)
		assert.Nil(t, cloned.Unique)
		assert.Nil(t, cloned.Limit)
		assert.Nil(t, cloned.Offset)
