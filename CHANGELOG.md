...
### Fixed
- `graphql.Type()` renaming an entity produced Go that did not compile. The GraphQL layer used the GraphQL type name wherever it emitted a Go identifier, including references to the entity struct — which velox's core generator names after the schema type. Renaming `User` to `Account` therefore generated edge methods, pagination and interface resolvers referring to a nonexistent `*Account`. Only the entity struct is now named by the schema type; everything velox generates alongside the SDL (Connection, Edge, PaginateOption, mutation inputs) keeps the GraphQL name, so both sides still agree. Generated output for every existing schema is byte-identical, since the two names coincide unless the annotation is used. Pinned by `contrib/graphql/type_rename_test.go`
- Review follow-ups on the interface-field feature and the loader: the collector no longer skips eager-loading a *renamed* edge for a `__typename`/`id` selection (renames resolve through the ordinary edge method and have no foreign-key fast path, so the skip traded one join for a query per row — the resolver's fast-path condition and the generated metadata now come from one helper and cannot drift); the fast path masks `NotFound`, so a target hidden by a privacy policy yields a null field instead of a GraphQL error; grouping to-many edges under an interface velox does not generate is rejected with a message naming the annotation, instead of emitting a `<Interface>Connection` with no such type; and an interface-field name whose Go form would collide with a member velox already emits on the entity (`Edges`, `Config`, a field or edge name) is rejected at generation time
- The loader's staleness check recognises a link step on Windows. It matched `"/link "` as a substring, which never matches the backslash-separated tool path `go build -n` prints there, so a stale loader binary was treated as current and generation ran against an outdated schema. It now matches the tool by base name
- `dialect/sql`'s set-operation builders record the SQLite "not supported" error once instead of on every render, and migration serialization also covers `NamedDiff` and `VerifyTableRange`, which mutate and read the same shared tables as `Create`
- `scripts/regen.sh` uses portable awk and fails loudly if the generated/handwritten classification yields nothing, instead of silently skipping all formatting while reporting success
- `TestMigrate_Diff` no longer races the wall clock: it recomputed the timestamped migration file name with `time.Now()` after running the diff, so a second boundary under load made it look for a file that was never written. It now finds the file by suffix
