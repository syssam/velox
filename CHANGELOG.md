...
- GraphQL `Bytes` fields could not be read or written: the generated SDL declared `scalar Bytes` without a Go binding, so gqlgen bound it to string and generated resolver stubs that panicked. The scalar now binds to `gqlrelay.MarshalBytes`/`UnmarshalBytes` (standard base64). Remove the generated `Thumbnail`-style stubs gqlgen moves to the end of your resolver files. Known limitation: reading NULL from a Nillable Bytes field (`*[]byte`) still fails inside gqlgen, which dereferences a nil pointer-to-slice before calling the marshaler. **Regenerate**
